...
Subroutine ADV_WQM
     ...
     & DLTXE, DLTYE
!
      Use MOD_HYDROVARS, Only: ZERO, VX, VY, ART1, ART2, DZ, DT, DT1, &
...
!
      Use MOD_WQM, Only: NAC, AC, C2, C1, C2F, DLT, DTM,  XYDF, &
...
!
  ...
  !  REAL(SP) :: C2MEAN(MLOC,KBM1,NCP)
      Real (SP), ALLOCATABLE :: C2MEAN (:,:,:) !0:MTLOC, KBM1, NCP)!LB 6may2016 turn MLOC into 0:MTLOC
...
!
  ...
  !  KURT GLAESEMANN 11 SEPT 2009 C2MEAN IS ZERO
      ALLOCATE(C2MEAN(0:MTLOC, 1:KBM1, 1:NCP))
      C2MEAN = 0.0
  !C2MEAN = 32.0 !LBtest
  !
  ...
  !
      Do I = 1, NCV
         ...
         Do K = 1, KBM1
        !           DTIJ(I, K) = DT1(I1)*DZ(K) !thickness of element I1   !LB - as it is in HYD -apr29,2016
      ...
      Do JCON = 1, NAC
        ...
        !
            Do I = 1, NCV_I
               ...
               IB = NIEC (I, 2)
           !
           ! KURT GLAESEMANN - 20% faster total run time -
...
!
           ...
           !EXFLUX=-UN*DTIJ(I,K)*((1.0+SIGN(1.0,UN))*FIJ2+                &
                !        (1.0-SIGN(1.0,UN))*FIJ1)*0.5+FXX+FYY
           !here finished Kurt's commenting
!
               UN = UVN (I, K)
           !
           ...
           !   VISCOF=XYDF*(FACT*(VISCOFF(IA)+VISCOFF(IB))*0.5 + FM1)
           ! KURT GLAESEMANN 10 SEPT 2009 - C2MEAN IS ZERO
           !TXX=0.5*(PFPXD(IA)+PFPXD(IB))*VISCOF  !LBtest
           !TYY=0.5*(PFPYD(IA)+PFPYD(IB))*VISCOF
  ...
  !
     ...
     !
         Do I=1,MLOC
            Do K = 1, KBM1
               If (K == 1) Then
                  TEMP = - WTS (I, K+1) * (C2(I, K, II)*DZ(K+1)+C2(I, &
                 & K+1, II)*DZ(K)) / (DZ(K)+DZ(K+1))
               Else If (K == KBM1) Then
                  TEMP = WTS (I, K) * (C2(I, K, II)*DZ(K-1)+C2(I, K-1, &
                 & II)*DZ(K)) / (DZ(K)+DZ(K-1))
               Else
                  TEMP = WTS (I, K) * (C2(I, K, II)*DZ(K-1)+C2(I, K-1, &
                 & II)*DZ(K)) / (DZ(K)+DZ(K-1)) - WTS (I, K+1) * (C2(I, &
                 & K, II)*DZ(K+1)+C2(I, K+1, II)*DZ(K)) / &
                 & (DZ(K)+DZ(K+1))
               End If
           !
           !
