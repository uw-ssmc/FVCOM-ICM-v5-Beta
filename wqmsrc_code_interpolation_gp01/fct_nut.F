...
Subroutine FCT_NUT
  ...
  !
      Use MOD_HYDROVARS, Only: DZ, THOUR
  ...
  !==============================================================================|
      Do JCON = 1, NAC
        ...
        !
           ...
           !
               If (K == 1) Then
                  NUTMAX = Max (NUTMAX, (C2(I, K, II)*DZ(K+1)+C2(I, &
                 & K+1, II)*DZ(K))/(DZ(K)+DZ(K+1)))
                  NUTMIN = Min (NUTMIN, (C2(I, K, II)*DZ(K+1)+C2(I, &
                 & K+1, II)*DZ(K))/(DZ(K)+DZ(K+1)))
               Else If (K == KBM1) Then
                  NUTMAX = Max (NUTMAX, (C2(I, K, II)*DZ(K-1)+C2(I, &
                 & K-1, II)*DZ(K))/(DZ(K)+DZ(K-1)))
               ...
               Else
                  NUTMAX = Max (NUTMAX, (C2(I, K, II)*DZ(K-1)+C2(I, &
                 & K-1, II)*DZ(K))/(DZ(K)+DZ(K-1)), (C2(I, K, &
                 & II)*DZ(K+1)+C2(I, K+1, II)*DZ(K))/(DZ(K)+DZ(K+1)))
                  NUTMIN = Min (NUTMIN, (C2(I, K, II)*DZ(K-1)+C2(I, &
                 & K-1, II)*DZ(K))/(DZ(K)+DZ(K-1)), (C2(I, K, &
                 & II)*DZ(K+1)+C2(I, K+1, II)*DZ(K))/(DZ(K)+DZ(K+1)))
               End If
