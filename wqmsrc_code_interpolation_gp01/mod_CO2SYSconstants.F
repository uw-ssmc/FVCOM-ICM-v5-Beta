...
!
    !allocation of variables to save DTTALK and DTTIC outputs
         ...
         DICUPT = 0.0 !DICUPT=-CP1-CP2  DIC uptake
         Allocate (DICBMP(0:MTLOC, KBM1))
         DICBMP = 0.0 !DICBMP=DOR1+DOR2 DIC source by basal met and photoresp
    ...
    !ALLOCATE(DICNIT(0:MTLOC,KBM1));   DICNIT  = 0.0  !DICNIT=-NT
         ...
         DICGAS = 0.0 !DICGAS=FLUXCO2  !only surface value is different to zero
         Allocate (DICSED(0:MTLOC, KBM1))
         DICSED = 0.0 !DICSED=BENDIC   !only bottom value is different to zero
!
...
!
         ...
         DICUPT_GL = 0.0 !DICUPT=-CP1-CP2  DIC uptake
         Allocate (DICBMP_GL(MGL, KBM1))
         DICBMP_GL = 0.0 !DICBMP=DOR1+DOR2 DIC source by basal met and photoresp
...
!
  ...
  !******************************************************************************!
      Subroutine CO2SYSCONSTANTS
!
         Use MOD_HYDROVARS, Only: ZZ, D
		 USE MOD_FILEINFO, ONLY: INK1K2
         Use MOD_WQM, Only: T, SALT, TALK, PO4 ! (Peng et al, PengCorrection of TALK)
         Implicit None
         Save
!
    !! need to update this one to define ph,pco2,dic,talk and probably all variables !WQM defines T, SALT, D
!
!
    ...
    ! Calculate constants inside a double DO loop
         Do K = 1, KBM1
            Do I = 1, MLOC
               ...
               Pbar = - D (I) * ZZ (K) / 10.0_SP !depth of this layer (positive,from surface) divided 10 to convert from m (dbar) to bar.
               S = SALT (I, K)
               sqrSal = Sqrt (S)
!
          !if(MSR)write(*,*)'D(I),ZZ(K),Pbar='
          !if(MSR)write(*,*)D(I),ZZ(K),Pbar
!
          !------------------------------------------------------------------------------
