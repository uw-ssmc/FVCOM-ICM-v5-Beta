...
!
     ...
     & TTL_MASS, TTL_MASS_GL  !LB added ADV,VDIF FLUXES 26jan2016
  !
...
!
  ...
  !
  IF(MSR) THEN
     OPEN(UNIT= 986, FILE = 'outputs/mss_blnc_data.dat', ACTION = 'WRITE')
   END IF
  !************************************************************************
  !**                          Begin Simulation                          **
  !************************************************************************
...
!
     ...
     !
     ! END 1 if defined block
     !			

IF(MOD(IINT,1800) == 0) THEN
        If (PAR) Then
            !------------------- Gathering total mass of solids ---------------------------------
            Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to check the tracer mass balance (not related WQ)
            & TTL_MASS(1), TTL_MASS_GL(1))
        END IF
END IF
    ...
    IF(MSR) THEN
      IF(MOD(IINT,1800) == 0) THEN
                WRITE(986,'(e10.3,2x,e10.3)') JDAY,SUM(TTL_MASS_GL(:))
                FLUSH(986)
      END IF
    END IF
...
!
           !
              ...
              !
                  Select Case (HISFN_SPLIT_BYLEVEL)
                  Case (.True.)
					! gp 24-Aug-2017 start of new reformatted code for readability and insert LDON, RDON, LPON, RPON output
					...
					& (D_GL(I), I=1, MGL), &
                    & (-D_GL(I)*ZZ(K), I=1, MGL), &
					& (C2_GL(I, K, 27), I=1, MGL), &
					& (C2_GL(I, K, 9), I=1, MGL), &
					& (C2_GL(I, K, 4), I=1, MGL), &
					& (C2_GL(I, K, 5), I=1, MGL), &
                    & (C2_GL(I, K, 13), I=1, MGL), &
					& (C2_GL(I, K, 14), I=1, MGL), &
					& (C2_GL(I, K, 20), I=1, MGL), &
                    & (total_netPP_GL(I), I=1, MGL), & 
					& (T_GL(I, K), I=1, MGL), &
					& (S_GL(I, K), I=1, MGL), &
					& (IAVG_GL(I, K), I=1, MGL), & 
					& (C2_GL(I, K, 10), I=1, MGL), & 
					& (C2_GL(I, K, 11), I=1, MGL), &
					& (C2_GL(I, K, 12), I=1, MGL), &
                    & (GPP_GL(I, K), I=1, MGL), &
					& (NPP_GL(I, K), I=1, MGL), &
                    & (C2_GL(I,K,16),I=1,MGL), &  ! LDON               
                    & (C2_GL(I,K,17),I=1,MGL), &  ! RDON               
	                & (C2_GL(I,K,18),I=1,MGL), &  ! LPON               
		            & (C2_GL(I,K,19),I=1,MGL), &  ! RPON               
					& (C2_GL(I, K, 33), I=1, MGL), &   ! DIC
					& (C2_GL(I, K, 34), I=1, MGL), &   ! TA
					& (pH_GL(I, K), I=1, MGL), &       ! pH
                    & (pCO2_GL(I, K), I=1, MGL), &     ! pCO2
					& (DICUPT_GL(I, K), I=1, MGL), &   ! DIC uptake by phyto (all DIC fluxes in mmolC/m3/day)
					& (DICBMP_GL(I, K), I=1, MGL), &   ! DIC source by basal metabolism and photorespiration
					& (DICPRD_GL(I, K), I=1, MGL), &   ! DIC source by predation
					& (DICMNL_GL(I, K), I=1, MGL), &   ! DIC source by mineralization of LDOC and RDOC
                    & (DICDEN_GL(I, K), I=1, MGL), &   ! DIC source by water column denitrification
					& (DICGAS_GL(I, K), I=1, MGL), &   ! DIC flux from air-sea gas exchange
					& (DICSED_GL(I, K), I=1, MGL), &   ! DIC flux from sediments to the water column
                    & (ADVFLUX_GL(I, K, 33), I=1, MGL), &   ! DIC adv + horiz diff flux
					& (VDIFFLUX_GL(I, K, 33), I=1, MGL), &  ! DIC vert diff flux
					& (ALKNH4_GL(I, K), I=1, MGL), &   ! TA sink by uptake NH4(all TALK fluxes in mmol/m3/day)
                    & (ALKNO3_GL(I, K), I=1, MGL), &   ! TA source by uptake NO3
					& (ALKNIT_GL(I, K), I=1, MGL), &   ! TA sink by nitrification
					& (ALKDEN_GL(I, K), I=1, MGL), &   ! TA source by denitrif
                    & (ALKREM_GL(I, K), I=1, MGL), &   ! TA source by prod NH4 by remin
					& (ALKNH4SED_GL(I, K), I=1, MGL), &   ! TA source from sedim
					& (ALKNO3SED_GL(I, K), I=1, MGL), &   ! TA flux from sedims to water column
                    & (ADVFLUX_GL(I, K, 34), I=1, MGL), &   ! TA adv + horiz diff flux
					& (VDIFFLUX_GL(I, K, 34), I=1, MGL)     ! TA vert diff flux
                 !
                 !
                 !write sediment diagenesis model outputs   !LBcomment
                    ...
                    !
                     Write (UNIT_HIS+K, '(:8(E14.6e3,1x),E14.6e3)') &
                       ...
                       & (JPON_GL(I, 1)/1000.0, I=1, MGL), &
					   & (JPON_GL(I, 2)/1000.0, I=1, MGL), &
					   & (JPON_GL(I, 3)/1000.0, I=1, MGL), &
					   & (JPOP_GL(I, 1)/1000.0, I=1, MGL), &
                       & (JPOP_GL(I, 2)/1000.0, I=1, MGL), &
					   & (JPOP_GL(I, 3)/1000.0, I=1, MGL), &
					   & (JPOS_GL(I)/1000.0, I=1, MGL), &
					   & (C2_GL(I, KBM1, 27), I=1, MGL), &
					   & (D_GL(I), I=1, MGL), &
					   & (T_GL(I, KBM1), I=1, MGL), &
                       & (C2_GL(I, KBM1, 13), I=1, MGL), &
					   & (C2_GL(I, KBM1, 14), I=1, MGL), &
					   & (C2_GL(I, KBM1, 20), I=1, MGL), &
					   & (C2_GL(I, KBM1, 29), I=1, MGL), &
                       & (CH40_GL(I), I=1, MGL), &
					   & (S_GL(I, KBM1), I=1, MGL), &
					   & (SODTM1S_GL(I), I=1, MGL), &
					   & (JNH4_GL(I)/1000.0, I=1, MGL), &
					   & (JNO3_GL(I)/1000.0, I=1, MGL), &
					   & (BENDEN_GL(I), I=1, MGL), &
					   & (JCH4_GL(I), I=1, MGL), &
					   & (JCH4G_GL(I), I=1, MGL), &
					   & (JHS_GL(I), I=1, MGL), &
					   & (JPO4_GL(I)/1000.0, I=1, MGL), &
                       & (JSI_GL(I)/1000.0, I=1, MGL), &
					   & (NH41_GL(I)/1000.0, I=1, MGL), &
					   & (NH42_GL(I)/1000.0, I=1, MGL), &
					   & (NO31_GL(I)/1000.0, I=1, MGL), &
                       & (NO32_GL(I)/1000.0, I=1, MGL), &
					   & (PO41_GL(I)/1000.0, I=1, MGL), &
					   & (PO42_GL(I)/1000.0, I=1, MGL), &
					   & (SI1_GL(I)/1000.0, I=1, MGL), &
					   & (SI2_GL(I)/1000.0, I=1, MGL), &
					   & (CH41TM1S_GL(I), I=1, MGL), &
                       & (CH42_GL(I), I=1, MGL), &
					   & (HS1_GL(I), I=1, MGL), &
                       & (HS2_GL(I), I=1, MGL), &
					   & (CPOC_GL(I, 1)/1000.0*2.667, I=1, MGL), &
					   & (CPOC_GL(I, 2)/1000.0*2.667, I=1, MGL), &
					   & (CPOC_GL(I, 3)/1000.0*2.667, I=1, MGL), &
					   & (CPON_GL(I, 1)/1000.0, I=1, MGL), &
                       & (CPON_GL(I, 2)/1000.0, I=1, MGL), &
					   & (CPON_GL(I, 3)/1000.0, I=1, MGL), &
					   & (CPOP_GL(I, 1)/1000.0, I=1, MGL), &
					   & (CPOP_GL(I, 2)/1000.0, I=1, MGL), &
                       & (CPOP_GL(I, 3)/1000.0, I=1, MGL), &
					   & (CPOS_GL(I)/1000.0, I=1, MGL), &
					   & (HSED1_GL(I)*1000.0, I=1, MGL), &
					   & (BENSTRTM1S_GL(I), I=1, MGL)!'BEN_STR'             dimensionless        dimensionless
					   ! gp 24-Aug-2017 end of new reformatted code for readability and insert LDON, RDON, LPON, RPON output
					   ! gp 24-Aug-2017 start of commenting out old code
					   ! !
...
! !f90ppr               &,(VDIFFLUX_GL(I,K,34),I=1,MGL)      ! TA vert diff flux
                 ...
                 ! !write sediment diagenesis model outputs   !LBcomment
					...
					! gp 24-Aug-2017 end of commenting out old code
					End If !end LBcomment
                 !
                     FLUSH (UNIT_HIS+K)
                  Case (.False.)
...
!--------------------- checking the mass balance of a tracer ------------------------------------
SUBROUTINE CheckMssTrc()
  USE MOD_LIMS, ONLY : MLOC, KBM1
  USE MOD_HYDROVARS, ONLY : D,DZ,ART1
  USE MOD_WQM, ONLY : TTL_MASS, C2, C1
  ...
  DO I=1, MLOC
    DO J=1, KBM1
      TTL_MASS(I) = TTL_MASS(I) + C2(I,J,3)*ART1(I)*D(I)*DZ(J)*0.001
      !TTL_MASS(I) = TTL_MASS(I) + C1(I,J,35)*ART1(I)*D(I)*DZ(J)*0.001
    END DO
  END DO
END SUBROUTINE
!-------------------------------------------------------------------------------------------------
