...
!
  ...
  !
      Use MOD_HYDROVARS, Only: ART, ART1, DZ, H, D, DT, DT1, EL, ET, &
     ...
     & HYDRO_GEOM_ALLOC, HYDRO_ALLOC, RHO !LB added RHO to call DENS2 8jan2016
  !
...
!
  ...
  Use MOD_WQM, Only: AHMDLT, FILGTH, JDAY, NAPL, NDLT, NSNP, TMEND, &
       ...
       & S3PTR, SAVPTR, SEDIMENT_CALC, BBMI, C1, C2, CTEMP, DFEEDI, EPI, &
       ...
       & T, SALT, DIC, TALK ,SEDTEMP_CALC !LB added this line to be able to call DENS2 8jan2016
  !LB   and also get DIC and TALK to be used in initialization of CO2star
!
  ...
  !
     ...
     & NXTFL, NXKFL, NXTVD, NXOPL, NXMBL, NXDIA, LEAFI, STEMI, ROOTI, &
     & TUBERI, MXDLT, MINSTEP, SNPC, RSIC, RSOC, RSOD, BCC, S1C, S2C, &
  ...
  !
     ...
     & SED_READ, SED_CALC
  !
      Use MOD_SAV, Only: NSAVM, ALEAF, AROOT, ASTEM, ATUBER, AEP, &
     ...
     & SAV_ALLOC, SAV_LOADS_ALLOC, SAV_READ
  !
...
91    Close (UNIT_LINKAGE)
  ...
  !
      ...
      DTI=DLTVAL(1) !Wen Long and LB, forced DTI to be the same as DLTVAL(1)
...
!
  ...
  !
      ...
      End If
!
  !IF DIC sedim flux calculated from CSOD, we must have SEDC=ON and BFC = OFF.
...
!
  !-------
  !LB sanity check: if only one of TALK/DIC are defined, define the other one
      If (ACC(33) == ' ON' .And. ACC(34) == 'OFF') Then
         NAC = NAC + 1
         AC (NAC) = 34
      End If
      If (ACC(33) == 'OFF' .And. ACC(34) == ' ON') Then
         AC (NAC) = 33 !Keep DIC at position 33
         NAC = NAC + 1
...
!
     ...
     !
         ...
         If (SAV_CALC) READ (ICI, 1030) LEAFI, STEMI, ROOTI, TUBERI, &
        & EPI
     !
...
1031  Format (/ / (8 X, 2 F8.0, I8, F8.2))
1032  Format (/)
         ...
         Do JCON = 1, NAC
            ...
            Do K = 1, KBM1
               Do I = 1, MTLOC
                 ...
                 & AC(JCON)) * ART1 (I) * D (I) * DZ (K) / 1000.0
               End Do
            End Do
         End Do
