...
!
      Use MOD_WQM, Only: pH, pCO2, TALK, DIC
!
...
!
    ...
    !!              is ignored in the pH calculation, assumed negligible  (we aren't modeling Si)
!
!
    !!**TALK and DIC must be in mol/kgSW but in the model they are in mmol/m3
    !!  Used to Assume density = 1000 kg/m^3 (such that mmol/m3=umol/kg)
    !!            and divide TALK and DIC by 10^6 to get mol/kg
    !!  NOW, use RHO to change mmol/m3 to umol/kg-SW (DIC/rho*10^3), and then divide by 10^6  (8 jan 2016)
    !
    !!start calculation
    !If(MSR) Write (*,*) 'LBnote A1. DIC,RHO,pH(at some surf place)= ',DIC(1,1),RHO(1,1),pH(1,1)
         Do K = 1, KBM1
            Do I = 1, MLOC
!
          !8jan16          TC = DIC(I,K) * 1.E-6_SP    !conversion of mmol/m3 to mol/kg,
          !8jan16          TA = TALK(I,K) * 1.E-6_SP   !assuming rho = 1000 kg/m3
               TC = DIC (I, K) / RHO (I, K) * 1.E-3_SP !conversion of mmol/m3 to mol/kg,
...
!
    !!--Keep CO2star at surface (to compute air-sea CO2 flux in calc_DIC_TALK.F)
       ...
       !CO2star_surf(I) = CO2star(I,1) * 1.e6_SP  !mmol/m3 (assume density=1000 kg-SW/m3) !LB commented on 8jan2016!
            auxLB = CO2star (I, 1) * RHO (I, 1) * 1.e3_SP !!LB: for some reason, CO2star_surf(I)= CO2star(I,1) * RHO(I,1) * 1.e3_SP in the same line doesn't give a correct result
            CO2star_surf (I) = auxLB !mmol/m3!
       !If(MSR) Write (*,*) 'LBnote A2. I= ',I,' CO2star(I,1), RHO(I,1)= ',CO2star(I,1), RHO(I,1)
       !If(MSR) Write (*,*) 'LBnote A2.      CO2star(I,1) * RHO(I,1) * 1.e3_SP= ',auxLB
       !If(MSR) Write (*,*) 'LBnote A2.      CO2star_surf(I)= ',CO2star_surf(I)
         End Do
!
    !If(MSR) Write (*,*) 'LBnote A3. pH(at some surf place)= ',pH(1,1), ' CO2star_surf(1)=',CO2star_surf(1),' CO2star(1,1)= ',CO2sta
         Return
