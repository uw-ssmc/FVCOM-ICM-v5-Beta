...
!
  ...
  !
    ...
    ! vjp 8/23/05   added these declarations
         Integer :: I, J, N ! Wen Long: N is never used here
    !
...
!
         Integer :: ITEMP, I, J, N
         Real (SP) :: ATN, DF, HSMOL, STOXI, GLIMSAV, GLIMEPI, DELLF
!
    !  ATN,     &
    ...
    !
       ...
       !
          ...
          !leaf growth rate (1/day)
               PLEAF (B, N) = PMSAV * FTPSAV (ITEMP) * GLIMSAV / &
          ...
          !epiphytes growth rate (1/day)
               PEP (B, N) = PMEPI * FTPEP (ITEMP) * GLIMEPI * DENLIM &
              & (B, N) / CCHLEPI !gC/gCHLA
          !==> PEP has unit 1/day
          !WL   !respiration rate 1/day
               BMEP (B, N) = BMEPI * FTREP (ITEMP) + PRSPEPI * PEP (B, &
              & N)!basal metabolism and photochemical respiration (1/day)
          !WL   !predation 1/day
