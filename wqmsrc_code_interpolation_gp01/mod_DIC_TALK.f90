...
Contains
  ...
  !   TOTINC      : Calculation of changes in DIC (DTTIC)                        !
...
!
    ...
    ! What parameterization do we use for air-sea exchange of O2 and CO2?     !1=reareation,  2=Wanninkhof (1992), 3=Nightingale (20
        ...
        &areation, 2=W92, 3=N00, 4=Ho06, 5=W12)'
...
!
            ...
            If (PAR) Then
               ...
               End Do
               Do I = 1, NHN !LB: I don't think pCO2atm is used in halo nodes,
...
!
         Use MOD_HYDROVARS, Only: DZ, D, RHO !LB added RHO Jan 8 2016
!
    USE MOD_WQM, ONLY :        &!
         B1,             &!
         B2,             &!
...
!
!
    !     USE MOD_ZOOP, ONLY:    AOCRSZ, AOCRLZ, DOSZ, DOLZ
!
...
!
          !********* Respiration
          !-- Fracion of losses by basal metabolism and photoresp that go to DIC
...
!
          !-- Fracion of losses by predation that go to DIC
               DOP1 = FDOP * PR1 (I, K) * DOXG (I, K) / (KHR1+DOXG(I, &
              & K))
               DOP2 = FDOP * PR2 (I, K) * DOXG (I, K) / (KHR2+DOXG(I, &
              & K))
          !    DOP3   = FDOP*PR3(I,K)*DOXG(I,K)/(KHR3+DOXG(I,K))
!
!
               If (COD_CALC) Then
             !-- Production of DIC by the oxydation of sulfide to sulfate (?) (COD)
          ...
          !DICUPT(I,K) = (-CP1-CP2)/12.*1000.0  !DIC uptake mmolC/m3/day
          !DICBMP(I,K) = (DOR1+DOR2)/12.*1000.0  !DIC source by basal metabolism and photorespiration
          !DICPRD(I,K) = (DOP1+DOP2)/12.*1000.0  !DIC source by predation
          !DICMNL(I,K) = (MNLLDOC(I,K) +MNLRDOC(I,K))/12.*1000.0  !DIC source by mineralization of LDOC and RDOC
          !DICDEN(I,K) = DENIT(I,K)/12.*1000.0  !DIC source by water column denitrification
          !LB: accumulate fluxes per DThistoryFile(t_his_dlt) period (integration will be finalized in wqm_main.F, wqm_output subrou
              ...
              & 1000.0 !DIC uptake mmolC/m3/d
               DICBMP (I, K) = DICBMP (I, K) + (DOR1+DOR2) / 12. * &
              & 1000.0 !DIC source by basal metabolism and photorespiration
               DICPRD (I, K) = DICPRD (I, K) + (DOP1+DOP2) / 12. * &
              & 1000.0 !DIC source by predation
               DICMNL (I, K) = DICMNL (I, K) + (MNLLDOC(I, &
              & K)+MNLRDOC(I, K)) / 12. * 1000.0 !DIC source by mineralization of LDOC and RDOC
               DICDEN (I, K) = DICDEN (I, K) + DENIT (I, K) / 12. * &
              & 1000.0 !DIC source by water column denitrification
	      !
          !!********* Change in TIC
!
          ...
          !            /12./86400.
               DTTIC (I, K) = (-CP1-CP2+DOR1+DOR2+DOP1+DOP2+MNLLDOC(I, &
              & K)+MNLRDOC(I, K)+DENIT(I, K)) / 12. / 86400. * 1000.0 !LB note: also added *1000 to get units of mmol/m3/s (instead of mol/m3/s)
...
!
          ...
          !
          !LB Jan 25 2016: Do not include nitrification as a sink of DIC. Rationale is that we are implicitly assuming that
          ...
          !
          !          !DICNIT(I,K) = - NT(I,K)/14./11.*1000.0  !DIC uptake by nitrification  mmolC/m3/day
          !          DICNIT(I,K) = DICNIT(I,K)- NT(I,K)/14./11.*1000.0  !DIC uptake by nitrification  mmolC/m3/day
...
!
	  ...
	  !******** Sediment flux
       ...
       !                         mmolC/m^3/d     gO2/m^2/d    mmolC/gO2        1/m
			...
			IF(D(I) > 0.0_SP)THEN
               BENDIC = CSODflux (I) * RCCO / (D(I)*DZ(KBM1))!CSODflux = CSOD + XJCNO3 in mod_sed.F
...
!
! Wen Long and Tarang try to incorporate BENDEN (denitrification in sediment
! diagensis for the bottom layer
!             DTTIC (I, KBM1) = DTTIC (I, KBM1) + BENDEN(I)/14.0 / 86400.0_SP *1000.0/(D(I)*DZ(KBM1))*106/84.8 
!mmolC/m^3/sec
!                                                 (gN/m^2/day )/14/86400  ==>  mole N/m^2/sec
!                                                 *1000                   ===> mmoles N/m^2/sec
!                                                 /(D*DZ)                 ===> mmoles N/m^3/sec
!                                                 *106/84.8               ===> mmoles C/m^3/sec

			   !DICSED(I,KBM1) = BENDIC  !DIC flux from sediments, snapshot
               DICSED (I, KBM1) = DICSED (I, KBM1) + BENDIC !DIC flux from sediments, accumulated
!
...
!
         End If
!
    !!NOTE: so far, if BENTHIC_FLUXES=true, we yet don't have BENDIC addition (no DIC from sediments)
!
...
!
         Use MOD_HYDROVARS, Only: DZ, D !CURRENT DEPTH
!
...
!
         Do I = 1, MLOC
			...
			IF(D(I) > 0.0_SP)THEN
				...
				alks_nh4sed = BENNH4 (I) / (D(I)*DZ(KBM1)) / 14. * 1000.0 !mmolN/m^3/day     !BENNH4 is in gN/m^2/day
				alks_no3sed = - BENNO3 (I) / (D(I)*DZ(KBM1)) / 14. * 1000.0 !mmolN/m^3/day     !BENNO3 is in gN/m^2/day
