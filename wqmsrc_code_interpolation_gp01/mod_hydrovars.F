...
Module MOD_HYDROVARS !Wen Long: this is very confusing. Modify to MOD_HYDROVARS
  ...
  !
      Use MOD_SIZES, Only: MGL, NGL, NOBTY, NCP !!LB added NCP 17may2016
  !
      Implicit None
      Save
  !
...
!
  ...
  !
      Real (SP), Allocatable :: XFLUX_OBC_WQM (:, :, :)!LB added may17,2016
  !
  !--------------------hydrodynamics-------------------------------------------------
  !
...
!
      Real (SP), Allocatable :: RHO (:, :)!LB: 8 jan 2016
  !
  ...
  !
     ...
     & hydro_filenumwidth, hydro_filenumstart, hydro_Nrec, hydro_dlt
      Namelist / wqm_history / HISFN, t_his_start, t_his_end, &
     & t_his_dlt, HISFN_SPLIT_BYLEVEL
      Namelist / wqm_stations / STNFN, Nstation, NstationNum_GL, &
...
Contains
  ...
  !
    ...
    !
         ...
         ELNC2 = 0.0
    !
         Allocate (RHO(0:MTLOC, KBM1))!LB 8jan2016
         RHO = 1000.0 !LB
    !
    ...
    !
         ...
         KH = 0.0
    !
         Allocate (XFLUX_OBC_WQM(0:NOBTY, KBM1, NCP))!LBadded May17,2016. Calculation occurs in adv_wqm.F and is then used in bcond_wqm.F
         XFLUX_OBC_WQM = 0.0 !LB                  as it was saved and used in FVCOM
    !
         Allocate (EL(0:MTLOC))
         EL = 0.0
         Allocate (ET(0:MTLOC))
         ...
         DTFA = 0.0
    !
    !Allocate (VISCOFH(0:NTLOC, KB))  !LB 9may16 commented this one and defined as below
  ...
  !
    ...
    !
         If (ALLOCATED(RHO)) DEALLOCATE (RHO)!LB 8 jan 2016
    !
    ...
    !
         If (ALLOCATED(XFLUX_OBC_WQM)) DEALLOCATE (XFLUX_OBC_WQM)!LB may17,2016
    !
