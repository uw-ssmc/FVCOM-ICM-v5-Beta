MODULE TXC_KNT
  ...
  USE MOD_SIZES, Only: MGL, NGL,NCP,NTXVB  ! NTXVB - no. of variables for toxics
  USE MOD_WQM, ONLY: SSI, C1, C2, POC_CONCENS, DOC_CONCENS, WMS, TxcOutVar, NumTxcOutVar, TxcOutput, &
                    & PcbCon_W, PcbCon_AlgW, PcbCon_ZplW, PcbCon_SldW, PcbCon_POCW,PcbConTW, PcbConTW_GL, &
                    ...
                    & PcbConTS1, PcbConTS1_GL, PcbConTS2, PcbConTS2_GL
...
! PRST2 - Porosity of layer 2
 ...
 CHARACTER(LEN = 1024) :: TXCF_INC,TXCF_PNT,TXCF_OBC, TXCF_INC_SD,FLAG_SD_INI, FLAG_WIND
 INTEGER, ALLOCATABLE, DIMENSION(:) :: TEMP, TEMP2, TEMP3, CHN_IDS
 REAL(SP) :: WNSSI, WNPOC, WINDVEL, HNRCFF
...
!----------------- Toxics concentrations in each constituents--------------------------------------------------
          ALLOCATE(PcbCon_W(MTLOC,KBM1,NTXVB))
          PcbCon_W = 0.0 
          ALLOCATE(PcbCon_AlgW(MTLOC,KBM1,NTXVB))
          PcbCon_AlgW = 0.0 
          ALLOCATE(PcbCon_ZplW(MTLOC,KBM1,NTXVB))
          PcbCon_ZplW = 0.0 
          ALLOCATE(PcbCon_SldW(MTLOC,KBM1,NTXVB))
          PcbCon_SldW = 0.0 
          ALLOCATE(PcbCon_POCW(MTLOC,KBM1,NTXVB))
          PcbCon_POCW = 0.0
          ...
          PcbConPOCW_GL = 0.0
          ALLOCATE(PcbConW_S1(MTLOC,NTXVB))
          PcbConW_S1 = 0.0 
          ALLOCATE(PcbConW_S2(MTLOC,NTXVB))
          PcbConW_S2 = 0.0 
          ALLOCATE(PcbCon_POCS1(MTLOC,NTXVB))
          PcbCon_POCS1 = 0.0 
          ALLOCATE(PcbCon_POCS2(MTLOC,NTXVB))
          PcbCon_POCS2 = 0.0 
          ALLOCATE(PcbCon_DOCS1(MTLOC,NTXVB))
          PcbCon_DOCS1 = 0.0 
          ALLOCATE(PcbCon_DOCS2(MTLOC,NTXVB))
          PcbCon_DOCS2 = 0.0 
          ALLOCATE(PcbCon_SldS1(MTLOC,NTXVB))
          PcbCon_SldS1 = 0.0 
          ALLOCATE(PcbCon_SldS2(MTLOC,NTXVB))
          PcbCon_SldS2 = 0.0 
          ...
          PcbConSldS2_GL = 0.0
          ALLOCATE(PcbConTW(MTLOC,KBM1))
          PcbConTW = 0.0 
          ALLOCATE(PcbConTS1(MTLOC))
          PcbConTS1 = 0.0 
          ALLOCATE(PcbConTS2(MTLOC))
          PcbConTS2 = 0.0
!----------------------------------------------------------------------------------
          ALLOCATE(SSDL(MTLOC))
          SSDL = 0.0
          ALLOCATE(FP_SSI_W(MTLOC,KBM1,NTXVB))
          FP_SSI_W = 0.0
          ALLOCATE(FP_POC_W(MTLOC,KBM1,NTXVB))
          FP_POC_W = 0.0
          ALLOCATE(FP_DOC_W(MTLOC,KBM1,NTXVB))
          FP_DOC_W = 0.0
          ALLOCATE(FP_BOC1_W(MTLOC,KBM1,NTXVB))
          FP_BOC1_W = 0.0
          ALLOCATE(FP_BOC2_W(MTLOC,KBM1,NTXVB))
          FP_BOC2_W = 0.0
          ALLOCATE(FP_ZOO_W(MTLOC,KBM1,NTXVB))
          FP_ZOO_W = 0.0
          ALLOCATE(FD_W(MTLOC,KBM1,NTXVB))
          FD_W = 0.0
          ALLOCATE(KP_SSI_W(MTLOC,KBM1,NTXVB))
          KP_SSI_W = 0.0
          ALLOCATE(KP_POC_W(MTLOC,KBM1,NTXVB))
          KP_POC_W = 0.0
          ALLOCATE(KP_DOC_W(MTLOC,KBM1,NTXVB))
          KP_DOC_W = 0.0
          ALLOCATE(KP_BOC1_W(MTLOC,KBM1,NTXVB))
          KP_BOC1_W = 0.0
          ALLOCATE(KP_BOC2_W(MTLOC,KBM1,NTXVB))
          KP_BOC2_W = 0.0
          ALLOCATE(KP_ZOO_W(MTLOC,KBM1,NTXVB))
          KP_ZOO_W = 0.0
!----------------------------------------------------------------------------------

          ALLOCATE(FP_SSI_S1(MTLOC,NTXVB))
          FP_SSI_S1 = 0.0
          ALLOCATE(FD_WS1(MTLOC,NTXVB))
          FD_WS1 = 0.0
          ALLOCATE(KP_SSI_S1(MTLOC,NTXVB))
          KP_SSI_S1 = 0.0
          ALLOCATE(KP_POC_S1(MTLOC,NTXVB))
          KP_POC_S1 = 0.0
          ALLOCATE(KP_DOC_S1(MTLOC,NTXVB))
          KP_DOC_S1 = 0.0
          ALLOCATE(FP_DOC_S1(MTLOC,NTXVB))
          FP_DOC_S1 = 0.0
          ALLOCATE(FP_POC_S1(MTLOC,NTXVB))
          FP_POC_S1 = 0.0

          ALLOCATE(FP_SSI_S2(MTLOC,NTXVB))
          FP_SSI_S2 = 0.0
          ALLOCATE(FD_WS2(MTLOC,NTXVB))
          FD_WS2 = 0.0
          ALLOCATE(KP_SSI_S2(MTLOC,NTXVB))
          KP_SSI_S2 = 0.0
          ALLOCATE(KP_POC_S2(MTLOC,NTXVB))
          KP_POC_S2 = 0.0
          ALLOCATE(KP_DOC_S2(MTLOC,NTXVB))
          KP_DOC_S2 = 0.0
          ALLOCATE(FP_DOC_S2(MTLOC,NTXVB))
          FP_DOC_S2 = 0.0
          ALLOCATE(FP_POC_S2(MTLOC,NTXVB))
          FP_POC_S2 = 0.0
...
!---------------------------------------------------------------------------------------------
          ...
          PSRT2 = 0.0
          ALLOCATE(DSPCB(MTLOC,NTXVB,2))
          DSPCB = 0.0
          ALLOCATE(DWPCB(MTLOC,KBM1,NTXVB))
          DWPCB = 0.0
          ...
          SSIWC_GL = 0.0
          ALLOCATE(CONPCB_W(MTLOC,KBM1,NTXVB))
          CONPCB_W = 0.0
          ALLOCATE(CONPCB_S1(MTLOC,NTXVB))
          CONPCB_S1 = 0.0
          ALLOCATE(CONPCB_S2(MTLOC,NTXVB))
          CONPCB_S2 = 0.0
          ...
          SSD_MSS = 0.0
          ALLOCATE(TXCHT1(MTLOC))
          TXCHT1 = 0.0
          ALLOCATE(TXCHT2(MTLOC))
          TXCHT2 = 0.0
...
SUBROUTINE READ_TXC_DATA()
 ...
 REAL(SP), ALLOCATABLE, DIMENSION(:,:) :: TEMPTXC
 CHARACTER(LEN = 64) :: FLGTXC_OUTPUT
 REAL (SP) :: TTIME, PTC_SD, PTC_WC, TXC_INIT_SD, UNIT_SCALE, UNIT_SCALE_SD, UNIT_SCL_LDN
 INTEGER :: NumLine
...
OPEN(UNIT = 378, FILE = 'inputs/txc_cnt.dat', ACTION = 'READ')
READ(378,*, IOSTAT = end_file)
IF(end_file < 0) THEN
	    WRITE (*,*) "Error occured - Problem with txc_cnt.dat"
	    STOP
END IF
 READ(378,*)TXCF_INC    ! input file of initial conditions for toxics
 READ(378,*)TXCF_PNT    ! input file of loading from point sources
 READ(378,*)TXCF_OBC    ! input file for open boundary conditions
 READ(378,*)TXCF_INC_SD ! file for txc initial conditions for sediment layer
 READ(378,*)
 READ(378,*)PTC_WC     ! partition coefficient for water column
 READ(378,*)PTC_SD     ! partition coefficient for sediment layer
 READ(378,*)SED_DEN   ! solid density of sediment layers
 KP_SSI_W = PTC_WC
 KP_SSI_S1 = PTC_SD
 KP_SSI_S2 = PTC_SD
 KP_SSI_W = 0.0 
 ...
 KP_POC_S2 = PTC_SD
 KP_DOC_S2 = PTC_SD
 READ(378,*)DFFCOEFS12   ! diff. coefficient for molecular diffusion between sdlyr 1 and sdlyr 2
 READ(378,*)BRLVEL       ! burial velocity of solids from sdlyr 2
 READ(378,*)
 READ(378,*)FLAG_SD_INI ! flag for initial txc distribution in sediment (constant/variable = 1/0)
 READ(378,*)TXC_INIT_SD     ! uniform txc concen if txc distribution in sediment is uniform
 READ(378,*)
 READ(378,*)NumCng         ! number of PCB congeners
 ALLOCATE(MolWghtCng(NumCng))   ! congener molar weight array
 DO I=1, NumCng
   READ(378,*)MolWghtCng(I)
 END DO
 READ(378,*)
 READ(378,*)HNRCFF
 READ(378,*)
 READ(378,*)FLGTXC_OUTPUT    ! Flag for outputting toxicant concentration data 
!------------------------------- initialization of toxics in sediment layer ----------------------------
 IF(TRIM(FLAG_SD_INI) == 'UNIFORM') THEN
   CONPCB_S1 = TXC_INIT_SD
   CONPCB_S2 = TXC_INIT_SD
 ELSE
   ...
   IF(end_file < 0) THEN
   	    ...
   	    STOP
   END IF
   ALLOCATE(TEMPTXC(MGL,NTXVB))
   DO I=1, MGL
     READ(211,*) (TEMPTXC(I,j), j=1,NTXVB)
   END DO
   !TEMPTXC = TEMPTXC*UNIT_SCALE
   TEMPTXC = TEMPTXC*UNIT_SCALE_SD*SED_DEN
   CLOSE(211)
   If (SERIAL) THEN
     CONPCB_S1 = TEMPTXC
     CONPCB_S2 = TEMPTXC
   END IF
    IF (PAR) THEN
          DO I = 1, MLOC
            CONPCB_S1 (I,:) = TEMPTXC (NGID(I),:)
            CONPCB_S2 (I,:) = TEMPTXC (NGID(I),:)
          END DO
          DO I = 1, NHN
            CONPCB_S1 (I+MLOC,:) = TEMPTXC (HN_LST(I),:)
            CONPCB_S2 (I+MLOC,:) = TEMPTXC (HN_LST(I),:)
          END DO
     END IF
    DEALLOCATE(TEMPTXC)
 END IF
...
END IF
TXCST = NCP - NTXVB + 1
ALLOCATE (RTMPX(0:MGL, KBM1, TXCST:NCP))         ! NTXVB - toxics related variables for water column
RTMPX = 0.0
DO I = 1, MGL
        DO K = 1, KBM1
...
ALLOCATE (RTEMP1(NBTX_PNT_GL, TXCST:NCP, TXQTIME))
...
ALLOCATE (TXCQDIS(NBTX_PNT, TXCST:NCP, TXQTIME))
...
HSED1 = 0.1
HSED = 0.9
!WRITE(*,*),NBTX_PNT,TXCQDIS
!WRITE(*,*)'----------------------------------'
!----------------------- information for outputing toxicant concentration data ----------------  
IF(TRIM(FLGTXC_OUTPUT) == 'TRUE') THEN 
  TxcOutput = .TRUE. 
  OPEN(UNIT = 216, FILE = 'inputs/txc_output_infor.dat', ACTION = 'READ') 
  NumLine = 0
  DO
    READ(216, *,IOSTAT=end_file)
    IF(end_file /= 0) THEN
      REWIND(216)
      EXIT
    ELSE
    ...
    END IF
  END DO
  NumTxcOutVar = NumLine
  ALLOCATE(TxcOutVar(NumLine))
  DO i=1,NumLine
    READ(216,*) TxcOutVar(i)
  END DO
  CLOSE(216)
ELSE 
  TxcOutput = .FALSE.
...
! TXCHT2 = 0.9
 ...
 DO I=1, MLOC
   ...
   END DO
      ...
      DO JJ=1,NTXVB
        TTL_MASS2(I) = TTL_MASS2(I) + CONPCB_S1(I,JJ)*ART1(I)*TXCHT1(I)*0.001
        TTL_MASS3(I) = TTL_MASS3(I) + CONPCB_S2(I,JJ)*ART1(I)*TXCHT2(I)*0.001
      END DO
...
IMPLICIT NONE
  ...
  REAL (SP), DIMENSION(MTLOC,KBM1,NTXVB) :: DMPCB_W
  REAL (SP), DIMENSION(MTLOC,NTXVB, 2) :: DMPCB_S
