...
Contains
  ...
  !
    ...
    !
         Use MOD_WQM, Only: SEDIMENT_CALC, SSI, V2, WSS, WSSHI, WSSNET,TxcKntOn !,        &!
    !
         Use MOD_LIMS, Only: MLOC, KBM1, MTLOC
         USE TXC_KNT, ONLY : SSDL, SSD_MSS
    !
         Use MOD_HYDROVARS, Only: DZ, DZ2D, D, ART1
    !
         Implicit None
         Integer :: I, K
         Real (SP), Dimension (0:MTLOC, KBM1), INTENT(OUT) :: DTSSI, FLXSSSI
         REAL (SP) :: Vel_rss, V_br
    ...
    !
       ...
       !
          ...
          IF(TxcKntOn == .TRUE.) THEN    
               SSDL(I) = ((WSSNET(I) * SSI (I, KBM1))/(D(I)*DZ2D(I,KBM1)) /86400.0)   ! settled sediment concentration change
          END IF     
				ELSE
					DTSSI (I, KBM1)=0.0_SP
				ENDIF
  ...
  !
    ...
    !
        ...
        & FRB2SZ, FRB3SZ, FRB1LZ, FRB2LZ, FRB3LZ, WWLZ1, WWLZ2, WWSZ1, WWSZ2
    ...
    !
         Do K = 1, KBM1
          ...
          !    & K) + UB3SZ * B3ASZ (I, K) + ULDSZ * LDOCASZ (I, K) + &
          !    & URDSZ * RDOCASZ (I, K) + ULPSZ * LPOCASZ (I, K) + URPSZ &
          !    & * RPOCASZ (I, K)
			   PRASZ (I, K) = UB1SZ * B1ASZ (I, K) + UB2SZ * B2ASZ (I, &
          ...
          !    & K) + UB3LZ * B3ALZ (I, K) + USZLZ * SZALZ (I, K) + ULLZ &
          !    & * LPOCALZ (I, K) + URLZ * RPOCALZ (I, K)
			   PRALZ (I, K) = UB1LZ * B1ALZ (I, K) + UB2LZ * B2ALZ (I, &
    ...
    !
         Do K = 1, KBM1
          ...
          !
              ...
              & K)-PRLZ(I, K)) / 86400.
          !------------------------------------------------------------------------------
            IF(ZOO_DVM == .TRUE.) THEN
                    IF((K>1) .AND. (K<KBM1)) THEN
                         CALL DVM_FUNC(I,K,WWSZ1,WWLZ1)
                         CALL DVM_FUNC(I,K-1,WWSZ2,WWLZ2)
                         DTSZ (I,K) = DTSZ (I,K) + (WWSZ1*SZ(I,K) - WWSZ2*SZ(I,K-1))/(D(I)*DZ2D(I,K))
                         DTLZ (I,K) = DTLZ (I,K) + (WWLZ1*LZ(I,K) - WWLZ2*LZ(I,K-1))/(D(I)*DZ2D(I,K))
                    END IF

                    IF (K == 1) THEN
                         CALL DVM_FUNC(I,K,WWSZ1,WWLZ1)
                         DTSZ (I,K) = DTSZ (I,K) + (WWSZ1*SZ(I,K))/(D(I)*DZ2D(I,K))
                         DTLZ (I,K) = DTLZ (I,K) + (WWLZ1*LZ(I,K))/(D(I)*DZ2D(I,K))
                    END IF

                    IF (K == KBM1) THEN
                         CALL DVM_FUNC(I,K-1,WWSZ1,WWLZ1)
                         DTSZ (I,K) = DTSZ (I,K) + (WWSZ1*SZ(I,K-1))/(D(I)*DZ2D(I,K))
                         DTLZ (I,K) = DTLZ (I,K) + (WWLZ1*LZ(I,K-1))/(D(I)*DZ2D(I,K))
                    END IF
            END IF
          !-----------------------------------------------------------------------------
    ...
    !
         Do K = 1, KBM1
          ...
          !
            End Do
         End Do
!----------------- Laki debugging - making effects on Si zero -----------------
         !SASZ = 0.0
         !SUSZ = 0.0
         !SALZ = 0.0
         !SULZ = 0.0
  ...
  !
    ...
    !
         If (SAV_CALC) Then
            Do I = 1, NSAVCELL
               B = SAVCELL (I)
               !DTLDOC (B, KBM1) = DTLDOC (B, KBM1) + &
              !& (LDOCSAVW(B)+LDOCEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRDOC (B, KBM1) = DTRDOC (B, KBM1) + &
              !& (RDOCSAVW(B)+RDOCEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTLPOC (B, KBM1) = DTLPOC (B, KBM1) + &
              !& (LPOCSAVW(B)+LPOCEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRPOC (B, KBM1) = DTRPOC (B, KBM1) + &
              !& (RPOCSAVW(B)+RPOCEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.

!------------------ temporary modification by Laki to accomadate SAV module --------------
              DO K=1,6
  ...
  !
    ...
    !
         If (SAV_CALC) Then
            DO I = 1, NSAVCELL
               B = SAVCELL (I)
               !DTNH4 (B, KBM1) = DTNH4 (B, KBM1) + &
              !& (NH4SAVW(B)+NH4EPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTNO3 (B, KBM1) = DTNO3 (B, KBM1) + &
              !& (NO3SAVW(B)+NO3EPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTLDON (B, KBM1) = DTLDON (B, KBM1) + &
              !& (LDONSAVW(B)+LDONEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRDON (B, KBM1) = DTRDON (B, KBM1) + &
              !& (RDONSAVW(B)+RDONEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTLPON (B, KBM1) = DTLPON (B, KBM1) + &
              !& (LPONSAVW(B)+LPONEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRPON (B, KBM1) = DTRPON (B, KBM1) + &
              !& (RPONSAVW(B)+RPONEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.

!----------------------- temporary modification added by Laki ----------------------
            DO K=1,6
  ...
  !
    ...
    !
         If (SAV_CALC) Then
          ...
          !          in the water column
          !
               !DTPO4 (B, KBM1) = DTPO4 (B, KBM1) + &
              !& (PO4SAVW(B)+PO4EPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
               !DTLDOP (B, KBM1) = DTLDOP (B, KBM1) + &
              !& (LDOPSAVW(B)+LDOPEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRDOP (B, KBM1) = DTRDOP (B, KBM1) + &
              !& (RDOPSAVW(B)+RDOPEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
            !   DTLPOP (B, KBM1) = DTLPOP (B, KBM1) + &
              !& (LPOPSAVW(B)+LPOPEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
              ! DTRPOP (B, KBM1) = DTRPOP (B, KBM1) + &
              !& (RPOPSAVW(B)+RPOPEPIW(B)) / (D(B)*DZ2D(B,KBM1)) / 86400.
...
!
         Use MOD_CO2SYS, Only: AIRSEA_OPTION, & !LB: tells what parameterization to use for both O2 and CO2 air-sea exchange
        & JDstart1
!
         Implicit None
         Real (SP) :: KRDO
...
!
	!---old way of calculating DOS and KRDO
          !CHLORN    = SALT(I,1)/1.80655
          !TDOS      = T(I,1)
          !DOS       = 14.5532+TDOS*(0.0054258*TDOS-0.38217)-CHLORN     &
          !             *(0.1665+TDOS*(9.796E-5*TDOS-5.866E-3))
	...
	!---new way of calcuting DOS and KRDO by Laura
...
!
          ...
          !     KRDO      = 0.157*RNU*(1.5*WMS)**1.5
              ...
              & * (0.1665+TDOS*(9.796E-5*TDOS-5.866E-3))
			  !if (I==200) then
			  !   Write (*,*) 'Opt 1 ,JDAY, KRDO= ',JDAY, KRDO
			  !End If
!
			Else If (JDAY > JDstart1(1) .AND. JDAY < JDstart1(2)) Then
                    FACTOR = 0.251 * WMS ** 2.0
          !Schmidt number for O2 (W92)
               Sc = 1953.4 - 128.0 * T (I, 1) + 3.9918 * T (I, 1) ** &
              & 2.0 - 0.050091 * T (I, 1) ** 3.0

		   !Schmidt number for O2 (W14)
           !  Sc = 1920.4 - 135.6 * T(I,1) + 5.2122 * T(I,1)**2.0          &
           !       - 0.10939 * T(I,1)**3.0 + 0.00093777 * T(I,1)**4.0
...
!
			  ...
			  !if (I==200) then
			  !   Write (*,*) 'Opt 5 ,JDAY, KRDO= ',JDAY, KRDO
			  !End If
			 End If
!
               DTDO (I, 1) = DTDO (I, 1) + KRDO / (D(I)*DZ2D(I,1)) * &
...
!------------------ Temporary modification by Laki for SAV module -------------------
              DTDO (B, KBM1) = DTDO (B, KBM1) + &
...
!------------------------------------------------------------------------------------
            ...
            End Do
         End If
    !Wen Long: SEDDOSAV should be also discounted from sediment layer
    !and SEDDOSAV should be added to SOD
    !directly and, and SOD finally enteres DTDO calculation instead
...
!------------------------------------------------------------------------------
SUBROUTINE DVM_FUNC(Node_ID,LAY_NO,Vel_SZ,Vel_LZ)

  USE MOD_WQM, ONLY: JDAY,ShpFacDVM,PeakTmDVM,SpanDVM,WmaxSZ,WmaxLZ,PrayLaySZ, &
                     & PrayLayLZ,SleepLaySZ,SleepLayLZ
  USE MOD_HYDROVARS, ONLY: D,ZZ

  IMPLICIT NONE

  REAL(SP), INTENT(OUT) :: Vel_SZ, Vel_LZ
  INTEGER, INTENT(IN) :: LAY_NO,Node_ID
  REAL(SP) :: tm,DepthRangeSZ,layer_depthSZ,DepthRangeLZ,layer_depthLZ

  tm = (JDAY - INT(JDAY))*24.0

  DepthRangeSZ = -(D(Node_ID)*ZZ(SleepLaySZ) - D(Node_ID)*ZZ(PrayLaySZ))
  DepthRangeLZ = -(D(Node_ID)*ZZ(SleepLayLZ) - D(Node_ID)*ZZ(PrayLayLZ))

  IF(DepthRangeSZ <= 0.0 .OR. DepthRangeLZ <= 0.0) THEN
     WRITE(*,*)'The zooplankton inhabitant depth range becomes negative'
     WRITE(*,*)'Please check the inputs to zooplankton DVM'
     STOP
  END IF
  
  layer_depthSZ = -(D(Node_ID)*ZZ(LAY_NO) - D(Node_ID)*ZZ(PrayLaySZ))
  layer_depthLZ = -(D(Node_ID)*ZZ(LAY_NO) - D(Node_ID)*ZZ(PrayLayLZ))
  
  IF(LAY_NO <= PrayLaySZ .OR. LAY_NO >= SleepLaySZ) THEN
     Vel_SZ = 0.0
  ELSE
     Vel_SZ = WmaxSZ*EXP(-((tm - PeakTmDVM)/SpanDVM)**ShpFacDVM)* & 
                         & (((tm - PeakTmDVM)/SpanDVM)**(ShpFacDVM - 1.0)) !WmaxSZ*(layer_depthSZ/DepthRangeSZ)*ShpFacDVM*
  END IF


  IF(LAY_NO <= PrayLayLZ .OR. LAY_NO >= SleepLayLZ) THEN
     Vel_LZ = 0.0
  ELSE
     Vel_LZ = WmaxLZ*EXP(-((tm - PeakTmDVM)/SpanDVM)**ShpFacDVM)* &
                         & (((tm - PeakTmDVM)/SpanDVM)**(ShpFacDVM - 1.0)) !WmaxLZ*(layer_depthLZ/DepthRangeLZ)*ShpFacDVM*
  END IF

END SUBROUTINE
!------------------------------------------------------------------------------
!If(MSR) Write (*,*) 'DVM velocity at node ',Node_ID,' layer ',LAY_NO,' is ',Vel_SZ,' m/s for small zooplankton and ',Vel_LZ,' m/s for large zooplankton'
