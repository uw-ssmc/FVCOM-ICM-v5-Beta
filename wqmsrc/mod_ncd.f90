...
Module MOD_NCD
  !
	  ...
	  USE MOD_WQM, ONLY : NumTxVrb, NumTxcOutVar, TxcOutVar, TxcKntOn, TxcOutput, SdmHT, TcrtConW, &
						& TcrtConSdm, TcrKntOn, NumTcrVrb, NumTcrOutVar, TcrOutVrb, TcrtOutput, C2, SPM, &
						& PcbCon_W, PcbCon_AlgW, PcbCon_ZplW, PcbCon_SldW, PcbCon_POCW, PcbCon_DOCW, &
						& PcbConW_S1, PcbCon_POCS1, PcbCon_DOCS1, PcbConW_S2, PcbCon_POCS2, PcbCon_DOCS2, BioAcc, &
						& PcbConWSZ, PcbConWLZ, PcbCon_AlgW1, PcbCon_AlgW2,POCSD,PONSD,POPSD

	  Implicit None
	  Save
!--Control Variables----------------------------------------------!
   logical :: cdf_out           	!!true to activate netcdf input/output
   integer :: nout_vars !=107        	!!number of variables to output !Wen Long:now hard wired
...
!--NetCDF IDs----------------------------------------------------!
   ...
   !integer :: ww_vid
   integer :: wts_vid
   INTEGER :: popsd_vid,pocsd_vid,ponsd_vid
...
!------------------------------- netcdf ids for Toxic related variables ------------------------- 
	INTEGER :: Pcbwc_vid, Pcbzpl_vid, Pcbagl_vid, Pcbsld_vid, Pcbws1_vid, Pcbws2_vid, Pcbpoc_vid, Pcbdoc_vid
	INTEGER :: Pcbpocs1_vid, Pcbpocs2_vid, Pcbdocs1_vid, Pcbdocs2_vid, PcbConsz_vid, PcbConlz_vid,Pcbagl1_vid,Pcbagl2_vid
	INTEGER :: Tcrwc_vid, Tcrsd_vid

	Integer ::	 T_vid 			&!(1:MGL, 1:KBM1)			! Wen Long, correction to assign values for all layers
				,SALT_vid		&! (1:MGL, 1:KBM1)		! Wen Long, correction to assign values for all layers
				!,D_vid 			&!(1:MGL)	&! Wen Long, add D_GL, EL_GL, H_GL
				!,EL_vid			&! (1:MGL)	&! Wen Long
				!,H_vid 			&!(1:MGL)	&! Wen Long
				,CCHL1_vid 		&!
				,total_netPP_vid&!
				...
				,TDIC_vid		&!C2 (1:MGL, 1:KBM1, 33)		!TDIC	!LB
				,TALK_vid		&!C2 (1:MGL, 1:KBM1, 34)		!TALK 	!LB
				,pH_vid			&!(1:MGL, 1:KBM1)			!pH		!LB
				,pCO2_vid		&!(1:MGL, 1:KBM1)		!pCO2	!LB
			    ,Calcite_vid    &
				,Aragonite_vid  &
				,P1_vid			&
				,P2_vid			&
...
!				,VDIFFLUX_vid	&
  ...
  !SIUPB,& !28 particulate biogenic silica (unavaiable ) concentration in water column (gSi/m^3) (why not call it SIUPB ??) !Wen Lon
  ...
  !TDIC,  & !33 Dissolved inorg carbon (mmolC/m^3)    				!LB
  !TALK, & !34 Total alkalinity (mmol/m^3) 		   		  			!LB
...
!------------------------------------------------------------------------------|
   ...
   if(map_type(1:1) /= "e" .and. map_type(1:1) /= "n")then ! .and. map_type(1:1) /= "b")then
...
!------------------------------------------------------------------------------|
 ...
 ! CDF_VDP  = u v wts kh el salt temp d CCHL1 total_netPP DOXG LDOC RDOC LPOC RPOC  B1 B2 NH4\ !NO3 LDON RDON LPON RPON PO4 TDIC TALK pH pCO2 P1 P2 BM1 BM2 NL1 NL2 PL1 PL2 B1SZ B2SZ B1LZ\ !B2LZ PR1 PR2 IAVG FI1 FI2 IK1 IK2 GPP NPP DDOC DCOD NITRIF DOSZ DOLZ DOPR DORALG RESP FTCOD\ !COD REAERDO BFLUX DICUPT DICBMP DICPRD DICMNL DICDEN DICGAS DICSED ALKNH4 ALKNO3 ALKNIT\ !ALKDEN ALKREM ALKNH4SED ALKNO3SED JPOC JPON JPOP JPOS CTEMP SODTM1S JNH4TM1S JNO3TM1S BENDEN\ !JCH4TM1S JCH4GTM1S JHSTM1S JPO4TM1S JSITM1S NH41TM1S NH42TM1S NO31TM1S NO32TM1S PO41TM1S\ !PO42TM1S SI1TM1S SI2TM1S CH41TM1S CH42TM1S HS1TM1S HS2TM1S CPOP CPON CPOC CPOS HSED1\ !BENSTRTM1S
...
!==============================================================================|
   ...
   integer, dimension(3) :: dynm3de_lev,dynm3de_lay,dynm3dobc_lay, dynm3txc_sed, dynm3tcr_sed 
   integer, dimension(4) :: dynm4txc_lay,dynm4tcr_lay
...
!--Define Fixed Model Dimensions
   ...
   ierr = nf90_def_dim(nc_ofid,"nine"   ,9      ,nine_did   )    !WLong added nine_did
   IF(TxcKntOn == .TRUE.) THEN
   		ierr = nf90_def_dim(nc_ofid,"NumtxcVar"   ,NumTxVrb     ,txc_did)      ! Laki, for toxics 
   END IF
   
   IF(TcrKntOn == .TRUE.) THEN
   		ierr = nf90_def_dim(nc_ofid,"NumTcrVar"   ,NumTcrVrb     ,tcr_did)      ! Laki, for tracers
   END IF
...
!--Set Up Data Dimensioning - Dynamic Vars
   ...
   dynm4txc_lay = (/node_did,siglay_did,txc_did,time_did/)
   dynm4tcr_lay = (/node_did,siglay_did,tcr_did,time_did/)
   dynm3txc_sed = (/node_did,txc_did,time_did/)
   dynm3tcr_sed = (/node_did,tcr_did,time_did/)
...
!WLong : the following are related to zoop
		case ( "B1SZ")
...
!
		...
		case("POCSD") !!=============== pocsd ======================================!
			ierr = nf90_def_var(nc_ofid,"POCSD",nf90_float,dynm2dn,pocsd_vid)
			ierr = nf90_put_att(nc_ofid,pocsd_vid,"long_name","Particulate organic carbon in sediment layer 1")
			ierr = nf90_put_att(nc_ofid,pocsd_vid,"units","gC meters-3")
			ierr = nf90_put_att(nc_ofid,pocsd_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,pocsd_vid,"type","data")
		case("PONSD") !!=============== ponsd ======================================!
			ierr = nf90_def_var(nc_ofid,"PONSD",nf90_float,dynm2dn,ponsd_vid)
			ierr = nf90_put_att(nc_ofid,ponsd_vid,"long_name","Particulate organic nitrogen in sediment layer 1")
			ierr = nf90_put_att(nc_ofid,ponsd_vid,"units","gN meters-3")
			ierr = nf90_put_att(nc_ofid,ponsd_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,ponsd_vid,"type","data")
		case("POPSD") !!=============== ponsd ======================================!
			ierr = nf90_def_var(nc_ofid,"POPSD",nf90_float,dynm2dn,popsd_vid)
			ierr = nf90_put_att(nc_ofid,popsd_vid,"long_name","Particulate organic phosphorus in sediment layer 1")
			ierr = nf90_put_att(nc_ofid,popsd_vid,"units","gP meters-3")
			ierr = nf90_put_att(nc_ofid,popsd_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,popsd_vid,"type","data")
		
!WLong: the following are related to CARBONATE_CALC
		case ( "TDIC")
		case ( "TALK")
		case ( "pH")
		case ( "pCO2")
...
!WLong: the following are related to SEDIMENT_CALC
		case ( "JPOC")
		case ( "JPON")
		case ( "JPOP")
		case ( "JPOS")
		...
		case ( "HSED1")
		case ( "BENSTRTM1S")
!Adi-2/14/18: the following are related to SOLIDS_CALC
		case ( "TSS")
		case ( "SSI")
		case ( "KESS")
		case ( "TURB")
		case ( "PARAD")
		!AN the following are related to SAV_CALC
		case ( "LEAF")
		case ( "STEM")
		case ( "ROOT")
		case ( "TUBER")
		case ( "EP")
		!AN the following are related to ZOO_CALC
		case ( "SZ")
		case ( "LZ")
		case ( "INSTCDFLX" )
		case ( "CO2star_sat" )
 ...
 IF(CARBONATE_CALC)THEN
		...
		case ("Calcite")
			ierr = nf90_def_var(nc_ofid,"Calcite",nf90_float,dynm3dn_lay,Calcite_vid)
			ierr = nf90_put_att(nc_ofid,Calcite_vid,"long_name","Calcite")
			ierr = nf90_put_att(nc_ofid,Calcite_vid,"standard_name","Calcite")
			ierr = nf90_put_att(nc_ofid,Calcite_vid,"units","")
			ierr = nf90_put_att(nc_ofid,Calcite_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Calcite_vid,"type","data")

		case ("Aragonite")
			ierr = nf90_def_var(nc_ofid,"Aragonite",nf90_float,dynm3dn_lay,Aragonite_vid)
			ierr = nf90_put_att(nc_ofid,Aragonite_vid,"long_name","Aragonite")
			ierr = nf90_put_att(nc_ofid,Aragonite_vid,"standard_name","Aragonite")
			ierr = nf90_put_att(nc_ofid,Aragonite_vid,"units","")
			ierr = nf90_put_att(nc_ofid,Aragonite_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Aragonite_vid,"type","data")
		case ( "DICUPT")
		...
		case ( "DICGAS")
			...
			ierr = nf90_put_att(nc_ofid,DICGAS_vid,"standard_name","Net CO2 flux dueto air-sea exchange")
			ierr = nf90_put_att(nc_ofid,DICGAS_vid,"units","kmolC")
			ierr = nf90_put_att(nc_ofid,DICGAS_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,DICGAS_vid,"type","data")
		case ( "INSTCDFLX" )
			ierr = nf90_def_var(nc_ofid,"INSTCDFLX",nf90_float,dynm3dn_lay,INSTCDFLX_vid)
			ierr = nf90_put_att(nc_ofid,INSTCDFLX_vid,"long_name","Instantaneous CO2 air-sea exhange flux")
			ierr = nf90_put_att(nc_ofid,INSTCDFLX_vid,"standard_name","INSTCDFLX")
			ierr = nf90_put_att(nc_ofid,INSTCDFLX_vid,"units","mmolC/m3/day")
			ierr = nf90_put_att(nc_ofid,INSTCDFLX_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,INSTCDFLX_vid,"type","data")
		case ( "DICSED")
		...
		case("CO2star_sat")
     		ierr = nf90_def_var(nc_ofid,"CO2star_sat",nf90_float,dynm2dn,CO2star_sat_vid)
     		ierr = nf90_put_att(nc_ofid,CO2star_sat_vid,"long_name","CO2* saturation concentration at surface")
     		ierr = nf90_put_att(nc_ofid,CO2star_sat_vid,"units","mmolC/m^3")
     		ierr = nf90_put_att(nc_ofid,CO2star_sat_vid,"grid","fvcom-grid")
     		ierr = nf90_put_att(nc_ofid,CO2star_sat_vid,"standard_name","surface CO2* saturation")
     		ierr = nf90_put_att(nc_ofid,CO2star_sat_vid,"type","data")
		case("CO2star_surf")
     		ierr = nf90_def_var(nc_ofid,"CO2star_surf",nf90_float,dynm2dn,CO2star_surf_vid)
     		ierr = nf90_put_att(nc_ofid,CO2star_surf_vid,"long_name","Surface CO2* concentration at surface")
     		ierr = nf90_put_att(nc_ofid,CO2star_surf_vid,"units","mmolC/m^3")
     		ierr = nf90_put_att(nc_ofid,CO2star_surf_vid,"grid","fvcom-grid")
     		ierr = nf90_put_att(nc_ofid,CO2star_surf_vid,"standard_name","surface CO2* concentration")
     		ierr = nf90_put_att(nc_ofid,CO2star_surf_vid,"type","data")
end select
ENDIF
...
ENDIF
	...
	IF (SOLIDS_CALC) THEN !Adi-2/14/18: Outputted solids
		select case(trim(cdf_vdp(i)))
		case ("TSS")
...
!----------------------------- Toxic related outputs --------------------------- 
IF((TxcKntOn == .TRUE.) .AND. (TxcOutput == .TRUE.)) THEN 
	DO i=1,NumTxcOutVar 
		SELECT CASE (TRIM(TxcOutVar(i)))
		CASE("PcbCon_W")
			ierr = nf90_def_var(nc_ofid,"PCBCONW",nf90_float,dynm4txc_lay,Pcbwc_vid)
			ierr = nf90_put_att(nc_ofid,Pcbwc_vid,"long_name","Dissolved PCB concentration in WC")
			ierr = nf90_put_att(nc_ofid,Pcbwc_vid,"standard_name","PcbCon_W")
			ierr = nf90_put_att(nc_ofid,Pcbwc_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbwc_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbwc_vid,"type","data") 
		CASE("PcbCon_ZplW")
			ierr = nf90_def_var(nc_ofid,"PCBCONZPLW",nf90_float,dynm4txc_lay,Pcbzpl_vid)
			ierr = nf90_put_att(nc_ofid,Pcbzpl_vid,"long_name","PCB concentration in zooplankton in WC")
			ierr = nf90_put_att(nc_ofid,Pcbzpl_vid,"standard_name","PcbCon_ZplW")
			ierr = nf90_put_att(nc_ofid,Pcbzpl_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbzpl_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbzpl_vid,"type","data") 
		CASE("PcbCon_AlgW")
			ierr = nf90_def_var(nc_ofid,"PCBCONALGW",nf90_float,dynm4txc_lay,Pcbagl_vid)
			ierr = nf90_put_att(nc_ofid,Pcbagl_vid,"long_name","PCB concentration in Algae in WC")
			ierr = nf90_put_att(nc_ofid,Pcbagl_vid,"standard_name","PcbCon_AlgW")
			ierr = nf90_put_att(nc_ofid,Pcbagl_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbagl_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbagl_vid,"type","data")
		CASE("PcbCon_AlgW1")
			ierr = nf90_def_var(nc_ofid,"PCBCONALGW1",nf90_float,dynm4txc_lay,Pcbagl1_vid)
			ierr = nf90_put_att(nc_ofid,Pcbagl1_vid,"long_name","PCB concentration in Algae in WC")
			ierr = nf90_put_att(nc_ofid,Pcbagl1_vid,"standard_name","PcbCon_AlgW1")
			ierr = nf90_put_att(nc_ofid,Pcbagl1_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbagl1_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbagl1_vid,"type","data")
		CASE("PcbCon_AlgW2")
			ierr = nf90_def_var(nc_ofid,"PCBCONALGW2",nf90_float,dynm4txc_lay,Pcbagl2_vid)
			ierr = nf90_put_att(nc_ofid,Pcbagl2_vid,"long_name","PCB concentration in Algae in WC")
			ierr = nf90_put_att(nc_ofid,Pcbagl2_vid,"standard_name","PcbCon_AlgW2")
			ierr = nf90_put_att(nc_ofid,Pcbagl2_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbagl2_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbagl2_vid,"type","data")		
		CASE("PcbCon_SldW")
			ierr = nf90_def_var(nc_ofid,"PCBCONSLDW",nf90_float,dynm4txc_lay,Pcbsld_vid)
			ierr = nf90_put_att(nc_ofid,Pcbsld_vid,"long_name","PCB concentration in soilds in WC")
			ierr = nf90_put_att(nc_ofid,Pcbsld_vid,"standard_name","PcbCon_SldW")
			ierr = nf90_put_att(nc_ofid,Pcbsld_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbsld_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbsld_vid,"type","data") 
		CASE("PcbCon_POCW")
			ierr = nf90_def_var(nc_ofid,"PCBCONPOCW",nf90_float,dynm4txc_lay,Pcbpoc_vid)
			ierr = nf90_put_att(nc_ofid,Pcbpoc_vid,"long_name","PCB concentration in POC in WC")
			ierr = nf90_put_att(nc_ofid,Pcbpoc_vid,"standard_name","PcbCon_POCW")
			ierr = nf90_put_att(nc_ofid,Pcbpoc_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbpoc_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbpoc_vid,"type","data")
		CASE("PcbCon_DOCW")
			ierr = nf90_def_var(nc_ofid,"PCBCONDOCW",nf90_float,dynm4txc_lay,Pcbdoc_vid)
			ierr = nf90_put_att(nc_ofid,Pcbdoc_vid,"long_name","PCB concentration in DOC in WC")
			ierr = nf90_put_att(nc_ofid,Pcbdoc_vid,"standard_name","PcbCon_DOCW")
			ierr = nf90_put_att(nc_ofid,Pcbdoc_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbdoc_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbdoc_vid,"type","data")
		CASE("PcbConW_S1")
			ierr = nf90_def_var(nc_ofid,"PCBCONWS1",nf90_float,dynm3txc_sed,Pcbws1_vid)
			ierr = nf90_put_att(nc_ofid,Pcbws1_vid,"long_name","Dissolved PCB concentration in Sediment layer 1")
			ierr = nf90_put_att(nc_ofid,Pcbws1_vid,"standard_name","PcbConW_S1")
			ierr = nf90_put_att(nc_ofid,Pcbws1_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbws1_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbws1_vid,"type","data") 
		CASE("PcbCon_POCS1")
			ierr = nf90_def_var(nc_ofid,"PCBCONPOCS1",nf90_float,dynm3txc_sed,Pcbpocs1_vid)
			ierr = nf90_put_att(nc_ofid,Pcbpocs1_vid,"long_name","PCB concentration in POC associated with Sediment layer 1")
			ierr = nf90_put_att(nc_ofid,Pcbpocs1_vid,"standard_name","PcbCon_POCS1")
			ierr = nf90_put_att(nc_ofid,Pcbpocs1_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbpocs1_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbpocs1_vid,"type","data")
		CASE("PcbCon_DOCS1")
			ierr = nf90_def_var(nc_ofid,"PCBCONDOCS1",nf90_float,dynm3txc_sed,Pcbdocs1_vid)
			ierr = nf90_put_att(nc_ofid,Pcbdocs1_vid,"long_name","PCB concentration in POC associated with Sediment layer 1")
			ierr = nf90_put_att(nc_ofid,Pcbdocs1_vid,"standard_name","PcbCon_POCS1")
			ierr = nf90_put_att(nc_ofid,Pcbdocs1_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbdocs1_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbdocs1_vid,"type","data")
		CASE("PcbConW_S2")
			ierr = nf90_def_var(nc_ofid,"PCBCONWS2",nf90_float,dynm3txc_sed,Pcbws2_vid)
			ierr = nf90_put_att(nc_ofid,Pcbws2_vid,"long_name","Dissolved PCB concentration in Sediment layer 2")
			ierr = nf90_put_att(nc_ofid,Pcbws2_vid,"standard_name","PcbConW_S2")
			ierr = nf90_put_att(nc_ofid,Pcbws2_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbws2_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbws2_vid,"type","data") 
		CASE("PcbCon_POCS2")
			ierr = nf90_def_var(nc_ofid,"PCBCONPOCS2",nf90_float,dynm3txc_sed,Pcbpocs2_vid)
			ierr = nf90_put_att(nc_ofid,Pcbpocs2_vid,"long_name","PCB concentration in POC associated with Sediment layer 2")
			ierr = nf90_put_att(nc_ofid,Pcbpocs2_vid,"standard_name","PcbCon_POCS2")
			ierr = nf90_put_att(nc_ofid,Pcbpocs2_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbpocs2_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbpocs2_vid,"type","data")
		CASE("PcbCon_DOCS2")
			ierr = nf90_def_var(nc_ofid,"PCBCONDOCS2",nf90_float,dynm3txc_sed,Pcbdocs2_vid)
			ierr = nf90_put_att(nc_ofid,Pcbdocs2_vid,"long_name","PCB concentration in POC associated with Sediment layer 2")
			ierr = nf90_put_att(nc_ofid,Pcbdocs2_vid,"standard_name","PcbCon_POCS2")
			ierr = nf90_put_att(nc_ofid,Pcbdocs2_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Pcbdocs2_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Pcbdocs2_vid,"type","data")
		END SELECT
	END DO
...
!----------------------------------------------------------------------------------------------------
IF((TxcKntOn == .TRUE.) .AND. (TxcOutput == .TRUE.) .AND. (BioAcc == .TRUE.)) THEN 
	DO i=1,NumTxcOutVar
		SELECT CASE (TRIM(TxcOutVar(i)))
			CASE("PcbConWSZ")
				ierr = nf90_def_var(nc_ofid,"PCBACCSZ",nf90_float,dynm4txc_lay,PcbConsz_vid)
				ierr = nf90_put_att(nc_ofid,PcbConsz_vid,"long_name","Accumulated PCB concentration in SZ")
				ierr = nf90_put_att(nc_ofid,PcbConsz_vid,"standard_name","PCBACCSZ")
				ierr = nf90_put_att(nc_ofid,PcbConsz_vid,"units","g/g OC")
				ierr = nf90_put_att(nc_ofid,PcbConsz_vid,"grid","fvcom_grid")
				ierr = nf90_put_att(nc_ofid,PcbConsz_vid,"type","data") 
			CASE("PcbConWLZ")
				ierr = nf90_def_var(nc_ofid,"PCBACCLZ",nf90_float,dynm4txc_lay,PcbConlz_vid)
				ierr = nf90_put_att(nc_ofid,PcbConlz_vid,"long_name","Accumulated PCB concentration in LZ")
				ierr = nf90_put_att(nc_ofid,PcbConlz_vid,"standard_name","PCBACCLZ")
				ierr = nf90_put_att(nc_ofid,PcbConlz_vid,"units","g/g OC")
				ierr = nf90_put_att(nc_ofid,PcbConlz_vid,"grid","fvcom_grid")
				ierr = nf90_put_att(nc_ofid,PcbConlz_vid,"type","data") 
		END SELECT
	END DO
END IF
!-----------------------------------------------------------------------------------------------
...
!----------------------------- tracer/pollutant related variables --------------------------------
IF((TcrKntOn == .TRUE.) .AND. (TcrtOutput== .TRUE.)) THEN 
	DO i=1,NumTcrOutVar
		SELECT CASE (TRIM(TcrOutVrb(i)))
		CASE("TcrtConW")
			ierr = nf90_def_var(nc_ofid,"TcrtConW",nf90_float,dynm4tcr_lay,Tcrwc_vid)
			ierr = nf90_put_att(nc_ofid,Tcrwc_vid,"long_name","Tracer concentration in the water column")
			ierr = nf90_put_att(nc_ofid,Tcrwc_vid,"standard_name","TcrtConW")
			ierr = nf90_put_att(nc_ofid,Tcrwc_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Tcrwc_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Tcrwc_vid,"type","data")
		CASE("TcrtConSdm")
			ierr = nf90_def_var(nc_ofid,"TcrtConSdm",nf90_float,dynm3tcr_sed,Tcrsd_vid)
			ierr = nf90_put_att(nc_ofid,Tcrsd_vid,"long_name","Tracer concentration in sediments")
			ierr = nf90_put_att(nc_ofid,Tcrsd_vid,"standard_name","TcrtConSdm")
			ierr = nf90_put_att(nc_ofid,Tcrsd_vid,"units","g meters-3")
			ierr = nf90_put_att(nc_ofid,Tcrsd_vid,"grid","fvcom_grid")
			ierr = nf90_put_att(nc_ofid,Tcrsd_vid,"type","data")
		END SELECT
	END DO 
END IF
...
!==============================================================================|
   ...
   USE MOD_WQM, only: t,salt,CCHL1,total_netPP,DOXG, LDOC, RDOC, 						&
	...
	USE MOD_CO2SYS, ONLY: 	DICUPT, DICBMP, DICPRD, DICMNL, DICDEN, DICGAS, &
							...
							ALKNH4SED,ALKNO3SED,INSTCDFLX, CO2star_sat, CO2star_surf, &
							Calcite,Aragonite


	USE MOD_ZOOP, ONLY: B1SZ, B2SZ, B1LZ, B2LZ, DOSZ, DOLZ, SZ, LZ

	USE MOD_OWQ, ONLY: IAVG, TSS, KESS, TURB, PARAD !AN
	!USE MOD_OWQ, ONLY: IAVG, KESS, TURB, PARAD
	USE MOD_WQMINIT, ONLY: CARBONATE_CALC,COD_CALC, ZOO_CALC

	USE MOD_SED, ONLY: SODTM1S
	USE MOD_SIZES, only: MGL, NGL, NSAVM!
	USE MOD_LIMS, only: MYID,NPROCS, MTLOC,IINT,KB, KBM1!,
...
!Wen Long the following are related to Zoop
		case ( "B1SZ")
...
!--
		...
		case ("POCSD") !!=============== pocsd ======================================!
			i1 = lbound(POCSD,1) ; i2 = ubound(POCSD,1)
			call putvar(i1,i2,MLOC,mgl,1,1,"n",POCSD,nc_ofid,pocsd_vid,myid,nprocs,ipt)
		
		case ("PONSD") !!=============== ponsd ======================================!
			i1 = lbound(PONSD,1) ; i2 = ubound(PONSD,1)
			call putvar(i1,i2,MLOC,mgl,1,1,"n",PONSD,nc_ofid,ponsd_vid,myid,nprocs,ipt)
		
		case ("POPSD") !!=============== popsd ======================================!
			i1 = lbound(POPSD,1) ; i2 = ubound(POPSD,1)
			call putvar(i1,i2,MLOC,mgl,1,1,"n",POPSD,nc_ofid,popsd_vid,myid,nprocs,ipt)
...
!Wen Long: the following are part of sediment module mod_sed
		case ( "JPOC" )
		case ( "JPON" )
		case ( "JPOP" )
		case ( "JPOS" )
	 ...
	 IF(CARBONATE_CALC)THEN
		...
		case ("Calcite")
			i1 = lbound(Calcite,1) ; i2 = ubound(Calcite,1)
			call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",Calcite,nc_ofid,Calcite_vid,myid,nprocs,ipt)

		case ("Aragonite")
			i1 = lbound(Aragonite,1) ; i2 = ubound(Aragonite,1)
			call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",Aragonite,nc_ofid,Aragonite_vid,myid,nprocs,ipt)
		case ( "DICUPT")
		...
		case ( "DICGAS")
			...
			call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",DICGAS,nc_ofid,DICGAS_vid,myid,nprocs,ipt)	!Wen Long, could be just 1 layer
		case ( "INSTCDFLX")

			i1 = lbound(INSTCDFLX,1) ; i2 = ubound(INSTCDFLX,1)
			call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",INSTCDFLX,nc_ofid,INSTCDFLX_vid,myid,nprocs,ipt)	!
		case ( "DICSED")
			i1 = lbound(DICSED,1) ; i2 = ubound(DICSED,1)
			call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",DICSED,nc_ofid,DICSED_vid,myid,nprocs,ipt)	!Wen Long, could be just 1 layer
		...
		case("CO2star_sat") 
			i1 = lbound(CO2star_sat,1) ; i2 = ubound(CO2star_sat,1)
			call putvar(i1,i2,MLOC,mgl,1,1,"n",CO2star_sat,nc_ofid,CO2star_sat_vid,myid,nprocs,ipt)
		
		case("CO2star_surf") 
			i1 = lbound(CO2star_surf,1) ; i2 = ubound(CO2star_surf,1)
			call putvar(i1,i2,MLOC,mgl,1,1,"n",CO2star_surf,nc_ofid,CO2star_surf_vid,myid,nprocs,ipt)
		case default
			!do nothing
		end select
	 ...
	 IF(SEDIMENT_CALC)THEN
      ...
      call putvar(i1,i2,MLOC,mgl,1,1,"n",JCH4GTM1S,nc_ofid,JCH4GTM1S_vid,myid,nprocs,ipt)  ! Laki modified - array was originally defined as 1D
      ...
      call putvar(i1,i2,MLOC,mgl,1,1,"n",JPO4TM1S,nc_ofid,JPO4TM1S_vid,myid,nprocs,ipt)  ! Laki modified - array was originally defined as 1D
      ...
      call putvar(i1,i2,MLOC,mgl,1,1,"n",NH41TM1S,nc_ofid,NH41TM1S_vid,myid,nprocs,ipt) ! Laki modified - array was originally defnined as 1D
...
IF((TxcKntOn == .TRUE.) .AND. (TxcOutput == .TRUE.)) THEN 
		...
		DO i=1,NumTxcOutVar 
			SELECT CASE (TRIM(TxcOutVar(i)))
			CASE("PcbCon_W")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbwc_vid,MLOC,KBM1,NumTxVrb,PcbCon_W,stck_cnt)

			CASE("PcbCon_ZplW")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbzpl_vid,MLOC,KBM1,NumTxVrb,PcbCon_ZplW,stck_cnt)

			CASE("PcbCon_AlgW")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbagl_vid,MLOC,KBM1,NumTxVrb,PcbCon_AlgW,stck_cnt)
			
			CASE("PcbCon_AlgW1")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbagl1_vid,MLOC,KBM1,NumTxVrb,PcbCon_AlgW1,stck_cnt)
			
			CASE("PcbCon_AlgW2")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbagl2_vid,MLOC,KBM1,NumTxVrb,PcbCon_AlgW2,stck_cnt)

			CASE("PcbCon_SldW")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbsld_vid,MLOC,KBM1,NumTxVrb,PcbCon_SldW,stck_cnt)

			CASE("PcbCon_POCW")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbpoc_vid,MLOC,KBM1,NumTxVrb,PcbCon_POCW,stck_cnt)

			CASE("PcbCon_DOCW")
				CALL PUTSVAR_TXC4D(nc_ofid,Pcbdoc_vid,MLOC,KBM1,NumTxVrb,PcbCon_DOCW,stck_cnt)

			CASE("PcbConW_S1")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbws1_vid,MLOC,NumTxVrb,PcbConW_S1,stck_cnt)

			CASE("PcbCon_POCS1")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbpocs1_vid,MLOC,NumTxVrb,PcbCon_POCS1,stck_cnt)

			CASE("PcbCon_DOCS1")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbdocs1_vid,MLOC,NumTxVrb,PcbCon_DOCS1,stck_cnt)

			CASE("PcbConW_S2")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbws2_vid,MLOC,NumTxVrb,PcbConW_S2,stck_cnt)
	
			CASE("PcbCon_POCS2")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbpocs2_vid,MLOC,NumTxVrb,PcbCon_POCS2,stck_cnt)

			CASE("PcbCon_DOCS2")
				CALL PUTSVAR_TXC3D(nc_ofid,Pcbdocs2_vid,MLOC,NumTxVrb,PcbCon_DOCS2,stck_cnt)
...
IF((TxcKntOn == .TRUE.) .AND. (TxcOutput == .TRUE.) .AND. (BioAcc == .TRUE.)) THEN 
		DO i=1,NumTxcOutVar 
			SELECT CASE (TRIM(TxcOutVar(i)))
				CASE("PcbConWSZ")
					CALL PUTSVAR_TXC4D(nc_ofid,PcbConsz_vid,MLOC,KBM1,NumTxVrb,PcbConWSZ,stck_cnt)
				CASE("PcbConWLZ")
					CALL PUTSVAR_TXC4D(nc_ofid,PcbConlz_vid,MLOC,KBM1,NumTxVrb,PcbConWLZ,stck_cnt)
			END SELECT
		END DO	
END IF
!----------------------------------------------------------------------------------------------------
...
IF((TcrKntOn == .TRUE.) .AND. (TcrtOutput == .TRUE.)) THEN 
		DO i=1,NumTcrOutVar 
			SELECT CASE (TRIM(TcrOutVrb(i)))
				CASE("TcrtConW")
					CALL PUTSVAR_TXC4D(nc_ofid,Tcrwc_vid,MLOC,KBM1,NumTcrVrb,TcrtConW,stck_cnt)

			    CASE("TcrtConSdm")
					CALL PUTSVAR_TXC3D(nc_ofid,Tcrsd_vid,MLOC,NumTcrVrb,TcrtConSdm,stck_cnt)
...
!==============================================================================!
SUBROUTINE PUTSVAR_TXC4D(FID,VARID,I1,I2,I3,TEMP,STKCNT)
  !============================================================================!
  !  Write static variables
  !==============================================================================!
  USE MOD_LIMS, Only: MYID, NPROCS, IINT, MTLOC, MLOC, KBM1
  # if defined (1)
  	USE MOD_PAR, Only: NMAP,GATHER_TXC, GATHER
  # endif
  USE MOD_WQM, ONLY: NumTxVrb, TxcKntOn, TcrKntOn, NumTcrVrb
  USE MOD_SIZES, ONLY: MGL

  IMPLICIT NONE
  INTEGER, INTENT(IN)             :: FID
  INTEGER, INTENT(IN)             :: VARID
  INTEGER, INTENT(IN)             :: I1, I2, I3, STKCNT
  REAL(sp),  INTENT(IN)             :: TEMP(I1,I2,I3)
  INTEGER                         :: IERR, NumVrb
  INTEGER, DIMENSION(4)           :: DIMS
  REAL(sp), ALLOCATABLE, DIMENSION(:,:,:) :: VRBLARRY, GTEMP
	...
	!call putvar(i1,i2,MLOC,mgl,kb-1,kb-1,"n",NH4,nc_ofid,NH4_vid,myid,nprocs,ipt)
	IF(NPROCS == 1) THEN 
		ALLOCATE(VRBLARRY(I1,I2,I3)) ; VRBLARRY(1:I1,1:I2,1:I3) = TEMP(1:I1,1:I2,1:I3)
	END IF

	!IF(TxcKntOn == .TRUE.) THEN
	!	NumVrb = NumTxVrb
	!ELSEIF(TcrKntOn == .TRUE.) THEN 
	!    NumVrb = NumTcrVrb 
	!END IF
	NumVrb = I3
	...
	IF(NPROCS > 1) THEN	
		ALLOCATE(GTEMP(MGL,I2,I3))
	 	CALL GATHER_TXC(0, MTLOC, MLOC, MGL, KBM1, NumVrb, MYID, NPROCS, NMAP, TEMP, GTEMP)    ! tracer/pollutant in water
		ALLOCATE(VRBLARRY(MGL,I2,I3)); VRBLARRY(1:MGL,1:I2,1:I3) = GTEMP(1:MGL,1:I2,1:I3)
		DEALLOCATE(GTEMP)
	END IF
...
IF(MYID /= 1) RETURN
  	...
  	IERR = NF90_PUT_VAR(FID,VARID,VRBLARRY,DIMS)

  IF(IERR /=NF90_NOERR)THEN
     WRITE(*,*)'error getting variable: '
     WRITE(*,*)TRIM(NF90_STRERROR(IERR))
	 OPEN (UNIT=850,FILE='outputs/array_values.dat', ACTION = 'WRITE')
	 DO ii=1,MGL

			WRITE(850,'(30(e10.4,3x))')(VRBLARRY(ii,jj,1),jj=1,KBM1)
  ...
  RETURN
END SUBROUTINE PUTSVAR_TXC4D
!==============================================================================!

!===============================================================================
SUBROUTINE PUTSVAR_TXC3D(FID,VARID,I1,I2,TEMP,STKCNT)
  !-----------------------------------------------------------------------------
  !  Write static variables
  !-----------------------------------------------------------------------------
  USE MOD_LIMS, Only: MYID, NPROCS, IINT, MTLOC, MLOC, KBM1
  ...
  INTEGER, INTENT(IN)             :: FID
  INTEGER, INTENT(IN)             :: VARID
  INTEGER, INTENT(IN)             :: I1, I2, STKCNT
  REAL(sp),  INTENT(IN)           :: TEMP(I1,I2)
  INTEGER                         :: IERR, NumVrb
  ...
  REAL(sp), ALLOCATABLE, DIMENSION(:,:) :: VRBLARRY, GTEMP
	...
	IF(NPROCS == 1) THEN 
		ALLOCATE(VRBLARRY(I1,I2)) ; VRBLARRY(1:I1,1:I2) = TEMP(1:I1,1:I2)
	END IF
	...
	IF(NPROCS > 1) THEN	
		ALLOCATE(GTEMP(MGL,I2))
	 	CALL GATHER(0, MTLOC, MLOC, MGL, NumVrb, MYID, NPROCS, NMAP, TEMP, GTEMP)    ! tracer/pollutant in water
		ALLOCATE(VRBLARRY(MGL,I2)); VRBLARRY(1:MGL,1:I2) = GTEMP(1:MGL,1:I2)
		DEALLOCATE(GTEMP)
	END IF
...
IF(MYID /= 1) RETURN
  ...
  IF(IERR /=NF90_NOERR)THEN
     WRITE(*,*)'error getting sediment variable: '
     WRITE(*,*)TRIM(NF90_STRERROR(IERR))
