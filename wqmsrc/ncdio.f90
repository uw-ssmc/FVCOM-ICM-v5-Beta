...
Subroutine NCD_READ_GRID (INFILE)
  ...
  !Real (SP), Allocatable, Dimension (:, :) :: TEMP !LBcleanup
  !----------------------------------------------------------------------------!
  !
...
Subroutine NCD_READ_SHAPE (INFILE)
  ...
  !---------------------------------------------------------------------
      ...
      Use MOD_HYDROVARS, Only: NVG, XG, YG, HG, Z, Z2DG, ZZ, DZ,DZ2D, DZZ, &
     & num_hyd_ints,SIGVAR
  !Wen Long took CONTROL out of MOD_HYDROVARS and put the used variables here
      Use MOD_CONTROL, Only: SERIAL, MSR, PAR
  !
      Use MOD_LIMS, Only: KB, KBM1
      Use MOD_SIZES, Only: NGL, MGL, HydVersionNum
      Use MOD_PREC, Only: SP
  !
      Implicit None
  !----------------------------------------------------------------------------!
...
& DTFAL, KHL, ELL, TL, SL, HO)
  ...
  !Wen Long took MOD_CONTROL out of MOD_HYDROVARS and put the used variables here
      Use MOD_CONTROL, Only: SERIAL, MSR, PAR !	&           !!TRUE IF 1 RUN
...
& DTFAL, KHL, ELL, TL, SL, HO)
  ...
  !
     ...
     !
         Do K = 1, KBM1
            Do I = 1, MLOC
                IF(SL_GL (NGID(I), K) < 0.00000001) THEN 
                    SL_GL (NGID(I), K) = 0.0
                END IF
        ...
        !
            Do I = 1, NHN
                IF(SL_GL (HN_LST(I), K) < 0.00000001) THEN 
                    SL_GL (HN_LST(I), K) = 0.0
                END IF
