...
!**                                                                    **
!**    Modified by Wen Long and confirmed same as James Martin/Greg    **
!**         Pellitiers Excel Results  12/22/2015                      **
!**                                                                    **
!************************************************************************
!**                                                                    **
!** Inputs                                                             **
...
Module MOD_SED
  ...
  & CSODflux,       &!LB: keep CSOD for later use in TOTINC (mod_DIC_TALK.F)
  & SIAT, JDAY, KADPO4, KADSA, NH4, NO3, PIP, PO4, SALT, T, SEDTEMP_CALC, &
  & TxcKntOn,POC_CONCENS, DOC_CONCENS
  !
...
!
  ...
  !    K0H1D,   &        !dissolved constituent reaction velocity of first  layer (m/d) (Wlong notes (3-32-7)(3-32-8)
  ...
  !
      Real (SP) :: ERROR, A1, AO2N !oxygen to nitogren ratio for denitrification (WLong (4-45)*(4-46)) (gO2/gN)
  !
  ...
  !WLong: Im surprised that these are not in the input controls.
...
!
  ...
  !
     ...
     & (:), HS1_GL (:), HS2_GL (:), HSED1_GL (:), HSED_GL(:)!,    &
  !	JPOC_GL(:,:),    &
  !                             JPON_GL(:,:),    &
  !                             JPOP_GL(:,:),    &
  !                             JPOS_GL(:),    &
  ...
  !
     ...
     & JPOCaccum (:, :), JPOSaccum (:)!accumulative POSi settlong flux(mgSi/m^2/day) positive into sediments
  !
...
Contains
  ...
  !********************************************************************************
    ...
    !
         If ( .Not. UNI_ICON_IN_SED_VAR) Then !LB: added this if.  IF UNI_ICON_IN_SED_VAR is true (ie, ICIC=WQ_SEDV in wqm_con.npt), then sediment initialization occured in wqm_inputs.F
  ...
  !********************************************************************************
    ...
    !
         ...
         MTVEL = 0.0 !WL moved from mod_wqm.F
...
!
    ...
    !
    !
999      Format (:///8 X, F8.0, I8, I8, I8, I8, I8)!LB added ,I8 for NDTSED
9999     Format (/)
...
1099     Format (://(8 X, I8, 3 F8.1))
    ...
    !
		 ...
		 !WLong canceled conversion,so input value should be prepared in m^2/sec
    !
    ...
    !
        ...
        & SSTATEIC == 1)!LB: if reading initial conds from ini file, automatically assume not steady state for ini conds
...
!
    ...
    !
	   ...
	   !
       !AN added NH4T2TM1S_SHARE and PO4T2TM1S_SHARE
       !
       !     If (SAV_CALC) Then
       !        NH4T2TM1S_SHARE (I) = NH4T2TM1S (I)
       !        PO4T2TM1S_SHARE (I) = PO4T2TM1S (I)
	   !	    HST2TM1S_SHARE (I) = CHS (I)
       !     End If
...
!
    ...
    !
       ...
       !
          ...
          !    & WB2NETMMD * ASC2 * B2 (I, KWC) + WB3NETMMD * ASC3 * B3 &
          !    & (I, KWC) + WPSINETMMD * SIUPB (I, KWC) + JPOSaccum (I)!Note that SIAT is not included for it's not organic
               JPOSaccum (I) = WB1NETMMD * ASC1 * B1 (I, KWC) + &
              & WB2NETMMD * ASC2 * B2 (I, KWC) &
              & + WPSINETMMD * SIUPB (I, KWC) + JPOSaccum (I)!Note that SIAT is not included for it's not organic
          !
  ...
  !
    ...
    !
    !Wen Long: We are not tracking TDIC in water column
...
!
         Real (SP) :: Ibreak !added by LB to report global node with SOD problems
!
    ...
    !         WS2NET(B) = WS2NET(B) + WS2SAV * SAVEFCT  !increase of settling rate (m/d) to alg 2
    !         WS3NET(B) = WS3NET(B) + WS3SAV * SAVEFCT  !increase of settling rate (m/d) to alg 3
    !         WSUNET(B) = WSUNET(B) + WSUSAV * SAVEFCT  !increase of settling rate (m/d) to particulate biogenic silicate (unavaiabl
    !       ENDDO
    !     ENDDO
    !   ENDIF
    !
    ...
    !due to deposition feeder
       ...
       !
            If (SAV_CALC) Then
              ...
              & HSED (I)!Wen Long: Added SEDNO3SAV here
...
!
       ...
       !
       !WLong   CH4SAT = 99.*(1.+(ZD(I,KWC)+D(I)*DZ2D(I,KWC)+HSED(I))/10.)*0.9759**STP20
...
!
       ...
       !
          ...
          !
              ...
              & (1.+(K3+W2)*DLTS_H2)!WLong notes (2-16) (DMD book 13.31, 13.33)
...
!
          ...
          !
             ...
             !
                 ...
                 & 2)*(DF_MORT(I)+DF_PRED(I))) / (XKPOC2+W2)!WLong (2-23-2)
             !
                  PON1 = (JPON(I, 1)-DF_GROW_POC1(I)/AMCN+&
                 & FRPON1*(DF_MORT(I)+DF_PRED(I))/AMCN) / (XKPON1+W2)!WLong (2-23-2)
             !
             !
                  PON2 = (JPON(I, 2)-DF_GROW_POC2(I)/AMCN+FRPON(I, &
                 & 2)*(DF_MORT(I)+DF_PRED(I))/AMCN) / (XKPON2+W2)!WLong (2-23-2)
             !
             !
                  POP1 = (JPOP(I, 1)-DF_GROW_POC1(I)/AMCP+&
                 & FRPOP1*(DF_MORT(I)+DF_PRED(I))/AMCP) / (XKPOP1+W2)!WLong (2-23-2)
             !
             !
                  POP2 = (JPOP(I, 2)-DF_GROW_POC2(I)/AMCP+FRPOP(I, &
                 & 2)*(DF_MORT(I)+DF_PRED(I))/AMCP) / (XKPOP2+W2)!WLong (2-23-2)
             !
...
!
       !
           ...
           & TF_SSTATE)
       !
            JSI = 0.0 !S*(SI1-SI0)  !LB: for now, just force JSi to be zero. Should improve it by forcing it only when ACC(silicate)=OFF
       !
!
       !
...
!
       ...
       !converted from mgSi/m^2/day to gSi/m^2/day (positive into water column)
            ...
            End If
       !
            CSODflux (I) = CSOD + XJCNO3 !LB: keep CSOD for later use in TOTINC (mod_DIC_TALK.F)
!
...
!
       ...
       !
           ...
           & H2 / 1.E6 !(kgN) Wen Long replaced SFA by ART1
            SEDMP = SEDMP + (POP1+POP2+POP3+PO4T2) * ART1 (I) * H2 / &
           & 1.E6 !(kgP) Wen Long replaced SFA by ART1
...
!------------- For PCB interaction with organic carbon ------------------------
            IF (TxcKntOn == .TRUE.) THEN
              POC_CONCENS(I, 1) = (POC1 + POC2 + POC3)*0.001  ! gC/m^3
              POC_CONCENS(I, 2) = (POC1 + POC2 + POC3)*0.001
              DOC_CONCENS(I, 1) = CH41*(12.0/32.0)     ! mgO2/L = mgC/L *(12/32)= gC/m^3
              DOC_CONCENS(I, 2) = CH42*(12.0/32.0)
            END IF
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------------------
  ...
  !
    ...
    !    see WLong(3-32-2), it is multiplied by FP1 anyway
       ...
       !
         Else
           ...
           & NH41 !WLong (4-40-1) with no KMNH4
         End If
         K3 = 0.
         J2 = 0.
         PIE1 = 0.
    ...
    !
        ...
        & TF_SSTATE)
         JNO3 = S * (NO31-NO30)!WLong (4-44)   !fixed to -0.011 in experiment 'PSMdiag_fixedJno3' by LB
!
    !
  ...
  !
    ...
    ! IERR &    !error flag, 1 : not able to find SOD solution within SODMIN and SODMAX
