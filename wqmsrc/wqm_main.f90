...
!
	 ...
	 & TUBER_GL, EP_GL, TxcKntOn, TxcOutput, NumTxVrb, TcrKntOn               !LB added ADV,VDIF FLUXES 26jan2016
...
!
  ...
  !
      ...
      USE TXC_KNT, ONLY: TXC_ALLOC, READ_TXC_DATA, PCB_KNT, &
                    ...
                    & CONPCB_S1,CONPCB_W, CONPCB_S2, ExtractTxcData, CONPCB_WGL, &
                    & CONPCB_S1GL, CONPCB_S2GL
...
!
  ...
  !
      ...
      Real (4) :: ELAPSED (2)
  !
      Character (Len=1024) :: TITLE_STN, TcrWCRestartFile, TcrSDRestartFile
  !
...
!-------------------------------------------------------------------------------
  ...
  !
    ...
    !------------------------- storing the global TCE areas for testing - added by Laki ------------------
                !  IF (PAR) THEN
                !       CALL GathExtra
                !  END IF
    !--------------------------------------------------------------------------------------
    ...
    !----------------------------------- Computations for toxics -----------------------------
      IF (TxcKntOn) THEN
              PTNTX_SLD = .TRUE.
              !IF(MSR) THEN
                !OPEN(UNIT= 975, FILE = 'outputs/tem_spl_data.dat', ACTION = 'WRITE')
               ! OPEN(UNIT= 976, FILE = 'inputs/transect_ids.dat', ACTION = 'READ')
                !READ(976,*) NOCHN_IDS
                !ALLOCATE(CHN_IDS(NOCHN_IDS))
               ! DO J=1,NOCHN_IDS
               !   READ(976,*) CHN_IDS(j)
               ! END DO
                !CLOSE(976)
...
IF (TxcKntOn) THEN
     ...
     numTxvb = NCP - NTXVB + 1
     !DO I=numTxvb, NCP
       !C2(:,:,I) = C2(:,:,I) + DWPCB(:,:,I)*DLT
     !END DO
     C2(:,:,numTxvb:NCP) = C2(:,:,numTxvb:NCP) + DWPCB(:,:,:)*DLT
     !CONPCB_W = C2(:,:,numTxvb:NCP)     ! Total PCB in the water column
     !CONPCB_W = CONPCB_W + DWPCB*DLT
     !CONPCB_S = CONPCB_S + DSPCB*DLT   ! Total PCB in sediment
     !C2(:,:,35) = CONPCB_W(:,:)
     !DO I=1,MLOC
       !CONPCB_W(I,K) = CONPCB_W(I,K) + DWPCB(I,K)*DLT
       !C2(I,:,35) = C2(I,:,35) + DWPCB(I)*DLT
     DO I=1,MLOC
       DO K=1,NTXVB
...
!
     ...
     USE TXC_KNT, ONLY: SSD_MSS, SSIWC_GL, SSDL_GL, PSRT1, PSRT1_GL, CONPCB_S1, CONPCB_W, &
                        ...
                        & PcbCon_SldS2, PcbConWS1_GL, PcbConPOCS1_GL, PcbConSldS1_GL, PcbConWS2_GL, &
                        & PcbConPOCS2_GL, PcbConSldS2_GL
      Implicit None
      Include "mpif.h"
  !
  ...
  !
     ...
     !
        ...
        !    Call GATHER (0, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !    & C2(0, 1, 35), C2_GL(1, 1, 35))    ! PCBW  - Laki

        !   IF(TxcKntOn == .TRUE.) THEN 
        !   !---------------- Gathering of toxics related variables ----------------------------
        !        Call GATHER (0, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & C2(0, 1, 35), C2_GL(1, 1, 35))    ! PCBW  - Laki
        ...
        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki gather SSI mass
        !         & SSD_MSS(1), SSDL_GL(1))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to gather porosity
        !         & PSRT1(1), PSRT1_GL(1))
        ...
        !         & PSRT2(1), PSRT2_GL(1))
          ...
          !------------------------------------------------------------------------------------
        !    DO J=1, NTXVB
        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to gather the toxic variables
        !         & CONPCB_S1(1, J), CONPCB_S1GL(1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to gather the toxic variables
        !         & CONPCB_S2(1, J), CONPCB_S2GL(1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & CONPCB_W(1, 1, J), CONPCB_WGL(1, 1, J)) 
        !
        !         Call GATHER (1, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & PcbCon_W(1, 1, J), PcbConW_GL(1, 1, J)) 

        !         Call GATHER (1, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & PcbCon_ZplW(1, 1, J), PcbConZplW_GL(1, 1, J)) 

        !         Call GATHER (1, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & PcbCon_SldW(1, 1, J), PcbConSldW_GL(1, 1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, KBM1, MYID, NPROCS, NMAP, &
        !         & PcbCon_POCW(1, 1, J), PcbConPOCW_GL(1, 1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbConW_S1(1, J), PcbConWS1_GL(1, J)) 

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbConW_S2(1, J), PcbConWS2_GL(1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbCon_POCS1(1, J), PcbConPOCS1_GL(1, J))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbCon_POCS2(1, J), PcbConPOCS2_GL(1, J)) 

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbCon_SldS1(1, J), PcbConSldS1_GL(1, J))  

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    
        !         & PcbCon_SldS2(1, J), PcbConSldS2_GL(1, J))               

        !    END DO
         !---------------------------------------------------------------------------------------
        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to check the tracer mass balance (not related WQ)
        !         & TTL_MASS2(1), TTL_MASS2_GL(1))

        !         Call GATHER (1, MTLOC, MLOC, MGL, 1, MYID, NPROCS, NMAP, &    ! added by Laki to check the tracer mass balance (not related WQ)
        !         & TTL_MASS3(1), TTL_MASS3_GL(1))
        !--------------------------------------------------------------------------------------
        !END IF 
...
!
     ...
     !
        ...
        ! Else
            JDAY_TMP = JDAY
        !End If
 PRINT_HIS = .FALSE.      ! temporary switching off the history files ---- by Laki
           ...
           !
              ...
              !
                 ...
                 !!                      &6,1x),F12.6))') JDAY_TMP, 4, MGL, (H_GL(I), &!
                    ...
                    !& MGL), 
                    & (C2_GL(I, K, 33), I=1, MGL), (C2_GL(I, K, &
              ...
              & JDAY_TMP
               ...
               Do I = 1, Nstation
                 ...
                 !                       &),F12.6))') I, NstationNum_GL (I), K, - D_GL &!
                    ...
                    & (NstationNum_GL(I), K), S_GL (NstationNum_GL(I), K)

                !    P1_GL (NstationNum_GL(I), K), P2_GL (NstationNum_GL(I), &
                ...
                !    & (NstationNum_GL(I), K), PR2_GL (NstationNum_GL(I), K), &
                !    & IAVG_GL (NstationNum_GL(I), K), DICUPT_GL &
        ...
        DO ii=1,KBM1
            DO jj=1,MLOC
                IF(ISNAN(C2(jj,ii,kk))) THEN
                    WRITE(*,*)"This variable goes to NAN",jj,ii,kk, DWTCR(jj,ii,kk)
                    STOP
                END IF
            END DO
        END DO
...
!!--------------------- gathering initial mass for testing - added by Laki ------------------
SUBROUTINE GatherTxcBactVrbs()
  ...
  USE TXC_KNT, ONLY: PcbCon_W, PcbCon_AlgW, PcbCon_ZplW, PcbCon_SldW, PcbCon_POCW, &
                     & PcbConW_GL, PcbConAlgW_GL, PcbConZplW_GL, PcbConSldW_GL, PcbConPOCW_GL, &
                     ...
                     & PcbConWS2_GL, PcbConPOCS2_GL, PcbConDOCS2_GL, PcbConSldS2_GL
  USE SC_TCR, ONLY: ConTcr, ConTcrSdm1,ConTcrSdm2, ConTcr_GL, ConTcrSdm1_GL, ConTcrSdm2_GL,TcrtConW, &
                     & TcrtConW_GL, TcrtConSdm, TcrtConSdm_GL
  IMPLICIT NONE

   IF(TxcKntOn) THEN

            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTxVrb, MYID, NPROCS, NMAP, & 
                                    & PcbCon_W(1, 1, 1), PcbConW_GL(1, 1, 1))    ! Pcb in water 
            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTxVrb, MYID, NPROCS, NMAP, & 
                                    & PcbCon_ZplW(1, 1, 1), PcbConZplW_GL(1, 1, 1))    ! Pcb in water 
            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTxVrb, MYID, NPROCS, NMAP, & 
                                    & PcbCon_AlgW(1, 1, 1), PcbConAlgW_GL(1, 1, 1))    ! Pcb in water 
            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTxVrb, MYID, NPROCS, NMAP, & 
                                    & PcbCon_SldW(1, 1, 1), PcbConSldW_GL(1, 1, 1))    ! Pcb in water 
            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTxVrb, MYID, NPROCS, NMAP, & 
                                    & PcbCon_POCW(1, 1, 1), PcbConPOCW_GL(1, 1, 1))    ! Pcb in water
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbConW_S1(1, 1), PcbConWS1_GL(1, 1))
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbCon_POCS1(1, 1), PcbConPOCS1_GL(1, 1))
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbCon_DOCS1(1, 1), PcbConDOCS1_GL(1, 1))
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbConW_S2(1, 1), PcbConWS2_GL(1, 1))
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbCon_POCS2(1, 1), PcbConPOCS2_GL(1, 1))
            Call GATHER (1, MTLOC, MLOC, MGL, NumTxVrb, MYID, NPROCS, &
                                    & NMAP, PcbCon_DOCS2(1, 1), PcbConDOCS2_GL(1, 1))
   END IF

   IF(TcrKntOn) THEN 

            Call GATHER_TXC(1, MTLOC, MLOC, MGL, KBM1, NumTcrVrb, MYID, NPROCS, NMAP, & 
                                    & TcrtConW(1, 1, 1), TcrtConW_GL(1, 1, 1))    ! tracer/pollutant in water 
            Call GATHER (1, MTLOC, MLOC, MGL, NumTcrVrb, MYID, NPROCS, NMAP, & 
                                    & TcrtConSdm(1, 1), TcrtConSdm_GL(1, 1))    ! tracer/pollutant in sediments 
...
SUBROUTINE MainSetup
  ...
  IF(MSR) THEN
    WRITE(*,*)'Starting FVCOM-ICM model run..........'
  END IF
  OPEN(UNIT = 377, FILE = 'inputs/WQM_welcome.dat', ACTION = 'READ')
  READ(377,*, IOSTAT = end_file)
  IF(end_file < 0) THEN
  	    WRITE (*,*) "Error occured - Problem with WQM_welcome.dat"
  	    STOP
  END IF

  READ(377,*) HydVersionNum     ! Version number of hydrodynamic model 
  READ(377,*) NumWqVrb    ! Number of main water-quality-variables except toxics
  READ(377,*) FLAG_TXC    ! switch for the Toxic kinetics: TRUE or FALSE
  READ(377,*) FLAG_TCR    ! switch for the Toxic kinetics: TRUE or FALSE
  READ(377,*) NumTxVrb    ! Number of toxics related variables
  READ(377,*) NumTcrVrb   ! Number of tracer variables

  IF(TRIM(FLAG_TXC) == 'ON') THEN
      TxcKntOn = .TRUE.
  ELSE
      TxcKntOn = .FALSE.
  END IF

  IF(TRIM(FLAG_TCR) == 'ON') THEN
      TcrKntOn = .TRUE.
  ELSE 
      TcrKntOn = .FALSE.
  END IF
 ...
 CLOSE(377)
  ...
  !WRITE(*,*)'The value of TxcKntOn', TxcKntOn
END SUBROUTINE
!--------------------------------------------------------------------------------
