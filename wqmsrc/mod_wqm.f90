...
Module MOD_WQM
  !
     ...
     & NSSFP, MGL, NGL, NOBTY, NSAVM, NTXVB
  !
      Use MOD_PREC, Only: SP, CDF_PREC
  !
      Use MOD_LIMS, Only: MTLOC, KBM1, KB, MLOC, MYID
  ...
  !
     ...
     & PIB1, PIB2, PIB3, TDIC, TALK, pH, pCO2,CbntIon,BiCbntIon, FP1CO2, FP2CO2
  !
  ...
  !pH,   & !-- pH (seawater scale)    									   !LB
  !pCO2    !-- partial pressure of CO2 in seawater (ppmv or microatm)     !LB
...
!
  ...
  !3D Spatially varying (horizontally and vertically) settling velocities (read from STLFN)
     ...
     & WS2, WS3, WSU, WSSHI, WSPM !fixed solids settling rate (m/day) when solids concentration is high
...
!
  ...
  !
     ...
     & BAOFN, DFOFN !WLong, Deposition feeder output
  !
  ...
  !
      Real (SP), Allocatable :: ADVFLUX (:, :, :)!LB: 26 jan 2016
      Real (SP), Allocatable :: VDIFFLUX (:, :, :)!LB: 26 jan 2016
      Real (SP), Allocatable :: ADVFLUX_GL (:, :, :)!LB: 26 jan 2016
      Real (SP), Allocatable :: VDIFFLUX_GL (:, :, :)!LB: 26 jan 2016
!
  !
  !RGL
  ...
  ! KURT GLAESEMANN add C2_GL
      ...
      Real (SP), Allocatable, Dimension (:) :: TTL_MASS, TTL_MASS_GL,M1DMass, ART1_GL, M1SDMass
      REAL (SP), ALLOCATABLE, DIMENSION (:,:) :: PCB_W, PCB_S
      REAL (SP), ALLOCATABLE, DIMENSION (:,:) :: POC_CONCENS, DOC_CONCENS
      REAL (SP), ALLOCATABLE, DIMENSION (:) :: TTL_MASS2, TTL_MASS2_GL
      REAL (SP), ALLOCATABLE, DIMENSION (:) :: TTL_MASS3, TTL_MASS3_GL
      REAL (SP), ALLOCATABLE, DIMENSION (:) :: PORST1, PORST2
  !          H_GL : still depth (global)
  !         EL_GL : surface elevation (global)
      Real (SP), Allocatable, Dimension (:, :) :: pH_GL, pCO2_GL !LB
  !LB: to save GPP and NPP in history output
...
!
  ...
  !
      ...
      REAL(SP), DIMENSION(:), ALLOCATABLE :: DLWCMCB
  !
      Real (SP), Allocatable, Dimension (:) :: BENDOC, BENNH4, BENNO3, &
     & BENDON, BENDOP, BENPO4, BENCOD, BENDO, BENSA, BENDEN, BENCH4G, &
     & BENCH4A
      Real (SP), Allocatable, Dimension (:) :: CSODflux !LB: CSODflux=CSOD gO2/m^2/day
  !
...
!
 ...
 LOGICAL :: TxcKntOn, TxcOutput    ! added by Laki for Toxics
 INTEGER :: NumWqVrb, NumTxVrb    ! added by Laki for Toxics
 ...
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: PcbConW_S1, PcbCon_POCS1, PcbCon_DOCS1, PcbCon_SldS1
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: PcbConWS1_GL, PcbConPOCS1_GL, PcbConDOCS1_GL, PcbConSldS1_GL 
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: PcbConW_S2, PcbCon_POCS2, PcbCon_DOCS2, PcbCon_SldS2
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: PcbConWS2_GL, PcbConPOCS2_GL, PcbConDOCS2_GL, PcbConSldS2_GL
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: TcrtConSdm, TcrtConSdm_GL
 REAL(SP), ALLOCATABLE, DIMENSION (:) :: PcbConTS1, PcbConTS1_GL, PcbConTS2, PcbConTS2_GL

 REAL(SP), ALLOCATABLE, DIMENSION (:, :, :) :: RdcnCon_W, RdcnCon_AlgW, RdcnCon_ZplW, RdcnCon_SldW, RdcnCon_POCW
 REAL(SP), ALLOCATABLE, DIMENSION (:,:) :: RdcnConTW, RdcnConTW_GL, SPM, DTSPM
 REAL(SP), ALLOCATABLE, DIMENSION (:, :, :) :: RdcnConW_GL, RdcnConAlgW_GL, RdcnConZplW_GL, RdcnConSldW_GL, RdcnConPOCW_GL
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: RdcnConW_S1, RdcnCon_POCS1, RdcnCon_DOCS1, RdcnCon_SldS1
 REAL(SP), ALLOCATABLE, DIMENSION (:, :) :: RdcnConWS1_GL, RdcnConPOCS1_GL, RdcnConDOCS1_GL, RdcnConSldS1_GL 
 ...
 REAL (SP) :: PTC_SD, PTC_WC,SED_DEN,DFFCOEFS12,BRLVEL,TxcSdmUnfm,CnbToDWLZ, LipidFracLZ,AlphaSZ,AlphaLZ,ExcLossSZ
 REAL (SP) :: HNRCFF, KuptkSZ, KuptkLZ, KowPCB, GrwthSZ, GrwthLZ,CnbToDWSZ, LipidFracSZ, VDiffCoff, KDecay,ExcLossLZ
 REAL (SP) :: OxgToCbnSZ,OxgToCbnLZ,OxgRspRateSZ,OxgRspRateLZ,WetToDryRtSZ, WetToDryRtLZ, EchToEoxSZ, EchToEoxLZ 
 REAL(SP), ALLOCATABLE, DIMENSION (:) :: MolWghtCng
 CHARACTER(LEN = 120) :: TxcSdmInitType
 LOGICAL :: PCBTxcOn,MTLTxcOn, TxcHotstart,BioAcc,DynUptkON
 INTEGER :: NumTxcOutVar
 CHARACTER(LEN = 30), ALLOCATABLE, DIMENSION (:) :: PCBOutVrb
 !---------------------------------- Tracer cmm parameters --------------------------------------
 CHARACTER(LEN = 1024) :: TCRF_INC,TCRF_PNT,TCRF_OBC,TCRF_INC_SD
 CHARACTER(LEN = 120) :: TcrSdmInitType
 REAL(SP) :: TcrSdmUnfm,KnBact,C0SalBact,AlphaBact,ThetaBact
 LOGICAL :: BACTERIAON,RADIONUCON,NOTCRKNTS
 REAL(SP), ALLOCATABLE, DIMENSION(:) :: RnKd, RnHalfTime
 INTEGER :: NumTcrOutVar
 CHARACTER(LEN = 30), ALLOCATABLE, DIMENSION (:) :: TCROutVrb
 !---------------------------------- sediment cmm parameters ------------------------------------
 REAL(SP) :: VonKrmn,Rgfh,PtcDensity,ErsnCoeff,CrtShear,SdmPrst
 !---------------------------------- Carbonate chemistry impact on Bio --------------------------
 REAL(SP) :: A_P1, A_P2, B_P1, B_P2, C_P1, C_P2, D_P1, D_P2
 !---------------------------------- zooplankton DVM parameters ---------------------------------
 REAL(SP) :: ShpFacDVM,PeakTmDVM,SpanDVM,WmaxSZ,WmaxLZ,PrayLaySZ,PrayLayLZ,SleepLaySZ,SleepLayLZ
 LOGICAL :: ZOO_DVM
...
!
         ...
         pCO2 = 0.0 !LB
         Allocate (CbntIon(0:MTLOC, KBM1))
         CbntIon = 0.0
         Allocate (BiCbntIon(0:MTLOC, KBM1))
         BiCbntIon = 0.0
         Allocate (FP1CO2(0:MTLOC, KBM1))
         FP1CO2 = 0.0
         Allocate (FP2CO2(0:MTLOC, KBM1))
         FP2CO2 = 0.0
!
    ...
    !
         ...
         SNFIX = 0.0
          ...
          ALLOCATE(TTL_MASS(MTLOC))      ! added by Laki check the mass balance of a tracer
          TTL_MASS = 0.0
          ALLOCATE(TTL_MASS2(MTLOC))      ! added by Laki check the mass balance of a tracer
          TTL_MASS2 = 0.0
          ALLOCATE(TTL_MASS3(MTLOC))      ! added by Laki check the mass balance of a tracer
          TTL_MASS3 = 0.0
...
!
    ...
    !
         ...
         DTM = 0.0
    !
         !Allocate (ADVFLUX(0:MTLOC, KBM1, NCP))!LB 26jan2016
         !ADVFLUX = 0.0 !LB
         !Allocate (VDIFFLUX(0:MTLOC, KBM1, NCP))!LB 26jan2016
         !VDIFFLUX = 0.0 !LB
...
!         Allocate (ADVFLUX_GL(MGL, KBM1, NCP))!LB 26jan2016
!         ADVFLUX_GL = 0.0 !LB
!         Allocate (VDIFFLUX_GL(MGL, KBM1, NCP))!LB 26jan2016
!         VDIFFLUX_GL = 0.0 !LB
!     ELSE
!         Allocate (ADVFLUX_GL(1, 1, NCP))!LB 26jan2016
!         ADVFLUX_GL = 0.0 !LB
!         Allocate (VDIFFLUX_GL(1, 1, NCP))!LB 26jan2016
!         VDIFFLUX_GL = 0.0 !LB
...
!
         If (ALLOCATED(TDIC)) DEALLOCATE (TDIC)!LB
         If (ALLOCATED(TALK)) DEALLOCATE (TALK)!LB
         If (ALLOCATED(pH)) DEALLOCATE (pH)!LB
         If (ALLOCATED(pCO2)) DEALLOCATE (pCO2)!LB
!
         If (ALLOCATED(pH_GL)) DEALLOCATE (pH_GL)!LB
         If (ALLOCATED(pCO2_GL)) DEALLOCATE (pCO2_GL)!LB
!
...
!
    ...
    !
         If (ALLOCATED(ADVFLUX)) DEALLOCATE (ADVFLUX)!LB: 26 jan 2016
         If (ALLOCATED(VDIFFLUX)) DEALLOCATE (VDIFFLUX)!LB: 26 jan 2016
         If (ALLOCATED(ADVFLUX_GL)) DEALLOCATE (ADVFLUX_GL)!LB: 26 jan 2016
         If (ALLOCATED(VDIFFLUX_GL)) DEALLOCATE (VDIFFLUX_GL)!LB: 26 jan 2016
    !
    ...
    !
         ...
         If (ALLOCATED(CSODflux)) DEALLOCATE (CSODflux)!LB
!
    ...
    !
    !LB GPP and NPP to be saved in history output
