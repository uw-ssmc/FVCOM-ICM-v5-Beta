...
!
  ...
  !			Wen LONG:  FPLEAF +FPROOT+FPSTEM+FPTUBER = 1
  ...
  !   & FTPREP !temperature control on predation on epiphyte ! AN: commented out not used anymore
  !
  ...
  !Note in 2002 version of ICM, EP unit was (gC-epiphytes/gC-leaf)
...
!
    ...
    !Production of SAV and Basal Metabolism of SAV
         ...
         BMTUBER = 0.0 !16
         ALLOCATE (BM_LEAF(MTLOC))    ! Added by Laki
         BM_LEAF = 0.0
    !
  ...
  !
    ...
    !
         ...
         Integer :: I, J, N, K ! Wen Long: N is never used here
    !
    ...
    !
         Do I = 1, NSAVCELL_GL !Loop through all SAV cells
            ...
            SAVNDS_GL (I) = B  ! Storing the global SAV-nodes for initialization by Laki
       !
...
1050     Format (/ / (8 X, A8, F8.0))
    ...
    !
	!
    !  AN: Commeted out. Replaced by f(T)
    !     Do J = - 50, 400
    !        TLOOK = FLOAT (J) / 10.
    !   !
...
5080     Format (/ ' SAV CELL ', I6, ', WQM CELL ', I6, ', # Species ', &
        & I6)
5082     Format (' Coverage Fraction ', 10 F8.1)!Wen Long changed from Width (m) to Coverage (dimensionless)
  ...
  !
    ...
    !
        ...
        & PO4T2TM1S_SHARE, HST2TM1S_SHARE, M2_SHARE, PIE2HS_SHARE
         Use MOD_CONTROL, Only: PAR, MSR !AN
         Use MOD_HYDROVARS, Only: EL !AN
         Implicit None
...
!
	...
	!	 KWC = 1 !water column layer (surface layer) !AN
		 KWL = 6
    ...
    !
      ...
      AveT = SUM(T(B,1:6))/6.0
       ...
       !
          ...
          !     ALPHSAV = 0.002 * SAVDPH (B, N) + 0.0016 !AN
    ...
    !
      ...
      AveSalt = SUM(SALT(B,1:6))/6.0
       ...
       !
            !ITEMP = 10. * T (B, KBM1) + 0.05 !WLong, changed to KBM1 !AN
       !
       ! COMPUTE RESPIRATION
       !
       ...
       !
          ...
          !BM_LEAF(B) = PMSAV * GET_FT (AveT, TRPMSAV, KTPS1, KTPS2)   ! added by Laki for DIC  interactions
