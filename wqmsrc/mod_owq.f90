...
Module MOD_OWQ
  ...
  !
	  ...
	  !Real (SP), Allocatable, Dimension (:, :) :: PARAD   ! Adi-2/13/18
	  Real (SP), Allocatable, Dimension (:, :) :: TSS, PARAD   ! Adi-2/13/18
  !
  !
Contains
  !
...
!
    ...
    !vjp 9/19/2005 added declarations
        ...
        & AVREFCOS
		 ...
		 Real (SP) :: TOTCHL, TOTPOC, TOTDOC, IATTOP, OPTDEPTH !Adi-2/13/18, Remove TSS
    !
    ...
    !
       ...
       ! Adi-2/13/18: calculate necessary variable which is not only surface layer
         Do I = 1, MLOC
       !     TOTCHL = (B1(I, 1)*CHLCMN1+B2(I, 1)*CHLCMN2+B3(I, &
       !    & 1)*CHLCMN3) * TCHL2CHL
			TOTCHL = (B1(I, 1)*CHLCMN1+B2(I, 1)*CHLCMN2) * TCHL2CHL !AN
       !
       ! EMPIRICAL FUNCTIONS FOR COLOR AND TURBIDITY
       !
            Do K = 1, KBM1
               !TOTPOC = B1 (I, K) + B2 (I, K) + B3 (I, K) + SZ (I, K) + LZ (I, K) + LPOC (I, K) + RPOC (I, K)
			   TOTPOC = B1 (I, K) + B2 (I, K) + SZ (I, K) + LZ (I, K) + LPOC (I, K) + RPOC (I, K) !AN B3
               TSS (I, K) = Max (SSI(I, K)+2.5*TOTPOC, 1.0) !original
               !TSS (I, K) = Max (SSI(I, K)+2.5*TOTPOC, 0.) !AN
               TURB (I, K) = Min (0.4*TSS(I, K), 12.)  !original
               !TURB (I, K) = Max (0.4*TSS(I, K), 0.)  !Avoid limit
               !TURB (I, K) = Max ((TSS(I, K)/1.6495)**(1/1.0055), 0.) ! Adi-2/16/18, for fine sed regrestion from papers.
  ...
  !
    ...
    !
         ...
         TURB = 0.0
		 Allocate (TSS(MTLOC, KBM1)) !Adi-2/13/18
         TSS = 0.0 !Adi-2/13/18
		 Allocate (PARAD(MTLOC, KBM1)) !Adi-2/13/18
         PARAD = 0.0 !Adi-2/13/18
         Allocate (IAVG(MTLOC, KBM1))
         IAVG = 0.0 !WLong moved this from mod_wqm.F
         Allocate (IATBOT(MTLOC, KBM1))
         IATBOT = 0.0 !WLong moved this from mod_wqm.F
         Allocate (KESS(MTLOC, KBM1))
         KESS = 0.0 !WLong moved this from mod_wqm.F
  ...
  !
      Subroutine OWQ_DEALLOC
		 ...
		 If (ALLOCATED(TSS)) DEALLOCATE (TSS) !Adi-2/13/18
		 If (ALLOCATED(PARAD)) DEALLOCATE (PARAD) !Adi-2/13/18
         If (ALLOCATED(IAVG)) DEALLOCATE (IAVG)!WLong moved this to here from wqm_main.F
         If (ALLOCATED(IATBOT)) DEALLOCATE (IATBOT)!WLong moved this to here from wqm_main.F
