...
!
    ...
    !===================================================================================|
         Do NMSG = 1, TRCV
           ...
           & SBUF)! Add RBUF,SBUF to fool compiler into NOT optimizing in a bug KURT GLAESEMANN 28 JAN 28 2010
            LPROC = STAT (MPI_SOURCE) + 1
            LP = CM(LPROC)%RCPT * NVARS * KT
            LBUF = CM(LPROC)%NRCV
            N2 = LP
...
!
  ...
  ! =============================================================================================== 
      ...
      Subroutine GATHER_TXC (N1, N2, NT, NTG, KT, NTXC, MYID, NPROCS, GM, A, AG)
    !
    !===================================================================================|
    !    GATHER ARRAY/VECTOR A INTO GLOBAL ARRAY/VECTOR                                 |
    !    LOCAL ARRAYS A(N1:N2,KT) --> AG(1:NTG,KT) BY MAPPING GM                        |
    ...
    !   include "mpif.h"
    !------------------------------------------------------------------------------
         Integer, Intent (In) :: N1, N2, NT, NTG, KT, MYID, NPROCS, NTXC
         Type (GMAP), Intent (In) :: GM (NPROCS)
         Real (SP), Intent (In) :: A (N1:N2, KT, NTXC)!source array bounds
         Real (SP), Intent (Out) :: AG (NTG, KT, NTXC)
    !------------------------------------------------------------------------------
         Real (SP), Allocatable :: SBUF (:, :, :), RBUF (:, :, :)
         INTEGER :: NN
         Integer STAT (MPI_STATUS_SIZE), IERR, I, IP, J, ISND, IRCV, &
    ...
    !------------------------------------------------------------------------------
      ...
      call GATHER(N1, N2, NT, NTG, KT* NTXC, MYID, NPROCS, GM, A, AG)      ! Kurt's suggesstion
      return
!
    !===================================================================================|
    !     CONFINE ARRAY TO INTERIOR ELEMENTS/NODES                                      |
    !===================================================================================|
         If (MYID /= 1) Then
            Allocate (SBUF(NT, KT, NTXC))
            Do I = 1, NT
               Do J = 1, KT
                DO NN = 1, NTXC
                  SBUF (I, J, NN) = A (I, J, NN)
                END DO
               End Do
            End Do
         End If
!
    ...
    !===================================================================================|
         If (MYID /= 1) Then
            ISND = MYID + 100
            Call MPI_SEND (SBUF, NT*KT*NTXC, MPI_F, 0, ISND, MPI_COMM_WORLD, &
           & IERR)
            Deallocate (SBUF)
            Return
         End If
...
!
         Do I = 1, GM(MYID)%NSIZE
            Do J = 1, KT
              DO NN = 1, NTXC
                 AG (GM(MYID)%LOC_2_GL(I), J, NN) = A (I, J, NN)
              END DO
            End Do
         End Do
    !===================================================================================|
...
!
         Do IP = 2, NPROCS
            NSZE = (GM(IP)%NSIZE)
            Allocate (RBUF(NSZE, KT, NTXC))
            IRCV = IP + 100
            Call MPI_RECV (RBUF, NSZE*KT*NTXC, MPI_F, IP-1, IRCV, &
           & MPI_COMM_WORLD, STAT, IERR)
            Do I = 1, GM(IP)%NSIZE
               Do J = 1, KT
                 DO NN = 1, NTXC
                    AG (GM(IP)%LOC_2_GL(I), J, NN) = RBUF (I, J, NN) 
                 END DO
               End Do
            End Do
            Deallocate (RBUF)
