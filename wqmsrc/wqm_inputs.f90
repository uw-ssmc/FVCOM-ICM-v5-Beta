...
!
     ...
     & MLOC, KB, KBM1
      Use MOD_SIZES, Only: MGL, NGL, NOBTY, NCP, NSAVM, NTXVB, NBTCR
  ...
  !
     ...
     & HYDRO_GEOM_ALLOC, HYDRO_ALLOC, RHO, HIS_OUTDIR,STN_OUTDIR,SIGVAR,SIGVARC !LB added RHO to call DENS2 8jan2016
  !
...
!
  ...
  Use MOD_WQM, Only: AHMDLT, FILGTH, JDAY, NAPL, NDLT, NSNP, TMEND, &
	   ...
	   & LEAF_GL, ROOT_GL, STEM_GL, TUBER_GL, EP_GL, TxcKntOn, TcrKntOn, &
       & HYDFN,NB,NSB,NQF,NL,KE,KEISS,KEVSS,KEDOC,&
       & IWCMN, IWCMP, IWCMS, JCS1MAX, JCS2MAX, JCS3MAX, NHQF, WQM_ALLOC, &
       & T, SALT, TDIC, TALK ,SEDTEMP_CALC !LB added this line to be able to call DENS2 8jan2016
  !LB   and also get TDIC and TALK to be used in initialization of CO2star
!
  ...
  !
     ...
     & SED_READ, SED_CALC
  !
  !     Use MOD_SAV, Only: NSAVM, ALEAF, AROOT, ASTEM, ATUBER, AEP, ! AN
  !
...
!
  ...
  !
     ...
     & ZOOP_READ
  !
      Use MOD_DIC_TALK, Only: PCO2AIR_READ_INITIAL !LB
  !
      Implicit None
      Integer :: J, JF, JG, JCON, K, L, N, NC_ID2
      Integer :: I, II, ISF, JP, JNP
      Integer :: JS1, JS2, JS3, TXCRT
      Integer :: YEARSTRT
     ...
     & :), RTMP23 (:, :), RTMP24 (:, :), RTMP25 (:, :), RTMP26 (:, :)
      Real (SP) :: TAasFsalt, auxS, auxT !LB 8jan2016
...
91    Close (UNIT_LINKAGE)
  ...
  !
	 ...
	 SIGVAR=.FALSE.
	  ...
	  DTI=DLTVAL(1)  !Long and LB, removed DTI
  ...
  !
      Call GENMAP !removed "IF(PAR)" by Wen Long
  !
  ...
  !
      Call WQMINIT_ALLOC
!
  !LB: allocate all CO2SYS parameters
  ...
  !
     ...
     & BAOC, SEDTC !WLong added BAOC, and SEDTC

	 SEDTC ='OFF'  !WLong set it to OFF if  SED_TEMP not defined
  !
  ...
  !
      Read (CON, 1065) BNDTC
  !
      TXCRT = NCP - NTXVB - NBTCR   ! added by Laki
      Read (CON, 1070) (ACC(JCON), JCON=1, TXCRT)
...
!
  ...
  !   UNI_ICON_IN      = ICIC == ' UNIFORM'              !MNOEL 2-5-93
      UNI_ICON_IN = ((ICIC == ' UNIFORM') .Or. (ICIC == ' WU_SEDV'))!LB: to read spatially variable sedims ini cond
      UNI_ICON_IN_SED_VAR = ICIC == ' WU_SEDV' !LB: to read spatially variable sedims ini cond
...
!------------------------------------------------------------------------------
!LB sanity check: if only one of TALK/TDIC are defined, define the other one
    IF ((ACC(33) == ' ON' .And. ACC(34) == 'OFF') .OR. &
       & (ACC(33) == 'OFF' .And. ACC(34) == ' ON')) THEN
       IF(MSR) THEN
         WRITE(*,*)'Both TALK and TDIC need to be ON for calculations'
         STOP
       END IF
    END IF
!------------------------------------------------------------------------------

!------------------- for regular WQ parameters - added by Laki------------------
      NAC = 0
      Do JCON = 1, TXCRT
         If (ACC(JCON) == ' ON') Then
            NAC = NAC + 1
...
!------------------- for TXC parameters - added by Laki ------------------------
  IF (TxcKntOn) THEN
    DO JCON = TXCRT+1, NCP-NBTCR
        NAC = NAC + 1
        AC (NAC) = JCON
    END DO
  END IF
...
!------------------- for Bacteria/Pollutant parameters - added by Laki ----------
  IF (TcrKntOn) THEN
    DO JCON = NCP-NBTCR+1, NCP
        NAC = NAC + 1
        AC (NAC) = JCON
    END DO
  END IF
...
!
      If (ACC(33) == ' ON') CARBONATE_CALC = .True. !LB
!
	  ! Get NASVM for matrix allocation AN
	  If (SAV_CALC) Then
...
!
  ...
  !
     ...
     & XFLUX_OBCNC1, DTFANC1, KHNC1, ELNC1, TNC1, SNC1, NTRECNC)!reading NTRECNC'th record in NCFILE
      ...
      EL = ELNC1
      D = H + EL
	  ET = ELNC1   !ET is used by vertvl.F
      DT = H + ET
  ...
  !
   ...
   !
     ...
     ! KRG 22-JUNE-2010, the above now in parallel, KURT GLAESEMANN
         ...
         Do I = 1, MGL
            Do K = 1, KBM1
               Read (ICI,*) (RTMP(I, K, JCON), JCON=1, TXCRT)
            End Do
         End Do
     !
         If (SERIAL) C1 = RTMP
...
!---------------------------------------------------------------------------------------
     ...
     !
      Else If (RES_ICON_IN) Then !MNOEL 2-5-93
         ...
         Read (RSI) OLDTITLE
     !
         Allocate (RTMP(0:MGL, KBM1, TXCRT))
         RTMP = 0.0 !WLong: note index from zero
  ...
  !
      If (BOUNDARY_CONC) Then
         Read (CBC, 1100)
         Read (CBC, 1020) (NCB(JCON), JCON=1, TXCRT)
         Read (CBC, 1100)
      End If
  !
  !***** Sources One
  ...
  !
      If (SOURCE_TWO) Then !MNOEL   1-25-93
         Read (S2, 1100)
         Read (S2, 1020) (S2LN(JCON), JCON=1, TXCRT)
         Do JCON = 1, NCP
  ...
  !
      If (SOURCE_THR) Then
         Read (S3, 1100)
         Read (S3, 1020) (S3LN(JCON), JCON=1, TXCRT)
         Do JCON = 1, NCP
  ...
  !
      If (SETTLING) Then
         ...
         Else
            ...
            RTMP2 = 0.0 !WLong: note index from zero because WSS index is from zero
            Allocate (RTMP21(0:MGL, KBM1))
            RTMP21 = 0.0 !WLong: note index from zero because WSL index is from zero
            Allocate (RTMP22(0:MGL, KBM1))
            RTMP22 = 0.0 !WLong: note index from zero because WSR index is from zero
            Allocate (RTMP23(0:MGL, KBM1))
            RTMP23 = 0.0 !WLong: note index from zero because WS1 index is from zero
            Allocate (RTMP24(0:MGL, KBM1))
            RTMP24 = 0.0 !WLong: note index from zero because WS2 index is from zero
            Allocate (RTMP25(0:MGL, KBM1))
            RTMP25 = 0.0 !WLong: note index from zero becasue WS3 index is from zero
...
11051 Format (://8 X, 9 F8.4)!go down 2 lines and write, but do not
  ...
  !     JDAY     = TMSTRT
      ...
      NWQMR = 0
      DLT = DLTVAL (DLTDP)
	  DTI = DLT			!WLong, use DLT for DTI all the time
      ELTMSPLT = JDAY * 86400.
      ELTMSTFL = JDAY * 86400.
      ELTMSKFL = JDAY * 86400.
  !
  ...
  !
      ...
      If (XY_DIFFUSION) Then
         Do K = 1, KBM1
            Do I = 1, MTLOC !LB 9may2016 changed from MLOC
               VISCOFH (I, K) = XYDF
            End Do
         End Do
      End If
  ...
  !
      If (SEDIMENT_CALC) Then
         If ( .Not. UNI_ICON_IN_SED_VAR) Then !LB: if UNI_ICON_IN_SED_VAR is true, SED_INIT called earlier
...
!
  ...
  !     & BALGAE_CALC !Wen Long: replaced SFA by ART1
!
  !***** Oxygen plots
!
