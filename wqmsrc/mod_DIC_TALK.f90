          ...
          IF((SAV_CALC) .AND. (K<=6)) THEN
               ...
               DO II=1,NSAVCELL
                  IF(I == SAVCELL(II)) THEN
                     CPSAV = 0.3*PLEAF(I,1)*LEAF(I,1)/(D(I)*DZ2D(I,K))   ! gC/m3   ! Added by Laki
                     EXIT              ! 0.3 was added as temporary solution to account DIC production due to respiration by SAV
                  END IF
               END DO
          ELSE
               CPSAV = 0.0
